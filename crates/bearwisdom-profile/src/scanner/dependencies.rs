...
pub fn detect_package_managers(
    ...
    for id in language_ids {
        ...
        for pm in lang.package_managers {
            ...
            let has_lock_file = pm
                .lock_file
                .map(|lf| root.join(lf).exists())
                .unwrap_or(false);

            let deps_installed = pm.deps_dir.map(|dd| root.join(dd).exists()).unwrap_or(true); // if no deps_dir declared, assume installed (e.g. cargo)
