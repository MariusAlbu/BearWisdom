...
fn cov_function_definition_as_rhs_emits_function() {
    ...
    assert!(
        sym.is_some(),
        "expected Function 'bar' from rhs function_definition; got: {:?}",
        r.symbols
    );
    assert_eq!(sym.unwrap().kind, SymbolKind::Function);
}
...
fn cov_variable_declaration_table_emits_class() {
    ...
    assert!(
        sym.is_some(),
        "expected Class 'MyModule' from table variable_declaration; got: {:?}",
        r.symbols
    );
...
fn cov_variable_declaration_no_init_emits_variable() {
    ...
    assert!(
        sym.is_some(),
        "expected Variable 'x' from uninit variable_declaration; got: {:?}",
        r.symbols
    );
...
fn cov_assignment_statement_function_emits_function() {
    ...
    assert!(
        sym.is_some(),
        "expected Function 'setup' from assignment_statement; got: {:?}",
        r.symbols
    );
    assert_eq!(sym.unwrap().kind, SymbolKind::Function);
}
...
fn cov_field_in_table_emits_field_or_method() {
    ...
    assert!(
        greet.is_some(),
        "expected symbol 'greet' from table field; got: {:?}",
        r.symbols
    );
    ...
    assert!(
        count.is_some(),
        "expected symbol 'count' from table field; got: {:?}",
        r.symbols
    );
...
fn cov_function_call_emits_calls() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "foo" && rf.kind == EdgeKind::Calls),
...
fn cov_method_index_expression_emits_calls() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "update" && rf.kind == EdgeKind::Calls),
...
fn cov_local_function_emits_function() {
    ...
    assert!(
        sym.is_some(),
        "expected Function 'helper' from local_function; got: {:?}",
        r.symbols
    );
    assert_eq!(sym.unwrap().kind, SymbolKind::Function);
}
...
fn cov_function_declaration_dot_index_emits_method() {
    ...
    assert!(
        sym.is_some(),
        "expected Method 'setup' from dot_index function_declaration; got: {:?}",
        r.symbols
    );
...
fn cov_function_declaration_method_index_emits_method() {
    ...
    assert!(
        sym.is_some(),
        "expected Method 'speak' from method_index function_declaration; got: {:?}",
        r.symbols
    );
    assert_eq!(sym.unwrap().kind, SymbolKind::Method);
}
...
fn cov_assignment_statement_table_emits_class() {
    ...
    assert!(
        sym.is_some(),
        "expected Class 'Animal' from global table assignment; got: {:?}",
        r.symbols
    );
    assert_eq!(sym.unwrap().kind, SymbolKind::Class);
}
...
fn cov_dot_index_call_emits_calls() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "save" && rf.kind == EdgeKind::Calls),
...
fn cov_assignment_dot_index_function_emits_method() {
    ...
    assert!(
        sym.is_some(),
        "expected Method 'render' from assignment dot_index; got: {:?}",
        r.symbols
    );
    assert_eq!(sym.unwrap().kind, SymbolKind::Method);
}
...
fn cov_setmetatable_emits_calls() {
    let r = extract::extract("Child = {}\nsetmetatable(Child, {__index = Parent})\n");
    // setmetatable must produce a Calls ref (the function is called)
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "setmetatable" && rf.kind == EdgeKind::Calls),
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "Parent" && rf.kind == EdgeKind::Inherits),
