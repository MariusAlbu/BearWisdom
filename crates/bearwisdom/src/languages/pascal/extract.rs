...
pub fn extract(source: &str) -> ExtractionResult {
    ...
    // `{$include 'helpers.inc'}` directives reference a source fragment whose
    // symbols extract under the fragment's own file path. Emit an Imports ref
    // per directive, keyed by the include file's bare stem, so the including
    // unit reaches those symbols through the wildcard file-stem rung.
...
fn extract_include_directives(src: &str, refs: &mut Vec<ExtractedRef>) {
