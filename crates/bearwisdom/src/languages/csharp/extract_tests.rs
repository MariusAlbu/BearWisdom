...
fn extracts_instantiation_edges() {
    ...
    assert!(r
        .iter()
        .any(|r| r.target_name == "Foo" && r.kind == EdgeKind::Instantiates));
}
...
class CatalogController {
    ...
    assert!(
        route.template.contains("catalog"),
        "Template: {}",
        route.template
    );
}
...
class CatalogDbContext : DbContext {
    ...
    assert!(result
        .db_sets
        .iter()
        .any(|d| d.entity_type == "CatalogItem"));
    assert!(result
        .db_sets
        .iter()
        .any(|d| d.entity_type == "CatalogBrand"));
}
...
"#;
    ...
    let y = symbols
        .iter()
        .find(|s| s.name == "Y" && s.kind == SymbolKind::Property);
...
"#;
    ...
    assert!(
        templates.contains(&"api/orders"),
        "Expected 'api/orders', got: {templates:?}"
    );
    assert!(
        templates.contains(&"api/orders/{orderId:int}"),
        "Expected 'api/orders/{{orderId:int}}', got: {templates:?}"
    );
    assert!(
        templates.contains(&"api/orders/cancel"),
        "Expected 'api/orders/cancel', got: {templates:?}"
    );
    assert_eq!(
        result.routes.len(),
        4,
        "Expected 4 routes, got: {templates:?}"
    );
}
...
"#;
    ...
    let templates: Vec<&str> = result.routes.iter().map(|r| r.template.as_str()).collect();
    assert!(
        templates.contains(&"api/catalog/items"),
        "Expected 'api/catalog/items', got: {templates:?}"
    );
    assert!(
        templates.contains(&"api/catalog/items/{id}"),
        "Expected 'api/catalog/items/{{id}}', got: {templates:?}"
    );
}
...
"#;
    ...
    assert!(
        templates.contains(&"api/catalog/items/{id:int}"),
        "Expected 'api/catalog/items/{{id:int}}', got: {templates:?}"
    );
}
...
"#;
    ...
    let templates: Vec<&str> = result.routes.iter().map(|r| r.template.as_str()).collect();
    assert!(
        templates.contains(&"/api/items"),
        "Expected '/api/items' preserved, got: {templates:?}"
    );
    assert_eq!(
        result.routes.len(),
        2,
        "Expected 2 routes, got: {templates:?}"
    );
}
...
"#;
    ...
    assert!(
        var_u.is_some(),
        "Expected variable symbol 'u' from lambda param. Symbols: {:?}",
        symbols
            .iter()
            .map(|s| (&s.name, s.kind))
            .collect::<Vec<_>>()
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Validate" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        var_u.is_some(),
        "Expected variable symbol 'u' from LINQ from_clause. Symbols: {:?}",
        symbols
            .iter()
            .map(|s| (&s.name, s.kind))
            .collect::<Vec<_>>()
...
"#;
    ...
    assert!(
        ...
        "Expected variable 'a' from switch pattern. Variables: {var_names:?}"
    );
    assert!(
        var_names.contains(&"s"),
        "Expected variable 's' from switch pattern. Variables: {var_names:?}"
    );
}
...
"#;
    let symbols = sym(src);
    let var_admin = symbols
        .iter()
        .find(|s| s.name == "admin" && s.kind == SymbolKind::Variable);
    assert!(
        var_admin.is_some(),
        "Expected variable symbol 'admin' from is-pattern binding. Symbols: {:?}",
        symbols
            .iter()
            .map(|s| (&s.name, s.kind))
            .collect::<Vec<_>>()
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Category" && r.kind == EdgeKind::TypeRef),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "DbConnection" && r.kind == EdgeKind::Instantiates),
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Open" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Database" && r.kind == EdgeKind::Instantiates),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Compute" && r.kind == EdgeKind::Calls),
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Default" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "DoWork" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "ProcessChecked" && r.kind == EdgeKind::Calls),
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "ProcessUnchecked" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "ReadMemory" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "ReadPtr" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Calc" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "GetName" && r.kind == EdgeKind::Calls),
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "GetRole" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "GetValues" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "SomeMethod" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Work" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "HandleClick" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        var_names.contains(&"s"),
        "Expected variable 's' from event handler lambda. Variables: {var_names:?}"
    );
    assert!(
        var_names.contains(&"e"),
        "Expected variable 'e' from event handler lambda. Variables: {var_names:?}"
    );
}
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "IsActive" && r.kind == EdgeKind::Calls),
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Transform" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        symbols
            .iter()
            .any(|s| s.name == "Category" && s.kind == SymbolKind::Property),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Check" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "ArgumentException" && r.kind == EdgeKind::TypeRef),
...
"#;
    ...
    assert!(
        s.iter()
            .any(|s| s.name == "~Resource" && s.kind == SymbolKind::Method),
...
"#;
    ...
    assert!(
        s.iter()
            .any(|s| s.name.starts_with("operator") && s.kind == SymbolKind::Method),
...
"#;
    ...
    assert!(
        s.iter()
            .any(|s| s.kind == SymbolKind::Method && s.name.contains("operator")),
...
"#;
    ...
    assert!(
        s.iter()
            .any(|s| s.name == "Click" && s.kind == SymbolKind::Event),
...
fn dump_csharp_node_kinds() {
    let snippets = [
        (
            "as_expression",
            "class S { void F(object x) { var y = x as Admin; } }",
        ),
        (
            "cast_expression",
            "class S { void F(object x) { var y = (Admin)x; } }",
        ),
        (
            "foreach",
            "class S { void F() { foreach (Admin item in items) {} } }",
        ),
        (
            "catch",
            "class S { void F() { try { } catch (ArgumentException e) { } } }",
        ),
        (
            "local_fn",
            "class S { void Outer() { int Inner(int x) => x + 1; } }",
        ),
