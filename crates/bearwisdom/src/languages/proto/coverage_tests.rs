...
fn cov_proto_message_parses_cleanly() {
    ...
    assert!(
        ...
        "proto message should parse without errors"
    );
}
...
fn cov_proto_enum_parses_cleanly() {
    ...
    assert!(
        !tree.root_node().has_error(),
        "proto enum should parse without errors"
    );
}
...
fn cov_proto_service_parses_cleanly() {
    ...
    assert!(
        !tree.root_node().has_error(),
        "proto service/rpc should parse without errors"
    );
}
...
fn cov_symbol_node_kinds_declared() {
    let plugin = ProtoPlugin;
    assert!(
        plugin.symbol_node_kinds().contains(&"message"),
        "message in symbol_node_kinds"
    );
    assert!(
        plugin.symbol_node_kinds().contains(&"service"),
        "service in symbol_node_kinds"
    );
    assert!(
        plugin.symbol_node_kinds().contains(&"enum"),
        "enum in symbol_node_kinds"
    );
    assert!(
        plugin.symbol_node_kinds().contains(&"field"),
        "field in symbol_node_kinds"
    );
}
...
fn cov_ref_node_kinds_declared() {
    ...
    assert!(
        plugin.ref_node_kinds().contains(&"import"),
        "import in ref_node_kinds"
    );
}
...
fn cov_message_emits_struct() {
    ...
    let sym = r
        .symbols
        .iter()
        .find(|s| s.name == "UserProfile" && s.kind == SymbolKind::Struct);
    assert!(
        sym.is_some(),
        "expected Struct 'UserProfile'; got: {:?}",
        r.symbols
    );
}
...
fn cov_rpc_emits_method() {
    ...
    let sym = r
        .symbols
        .iter()
        .find(|s| s.name == "SayHello" && s.kind == SymbolKind::Method);
    assert!(
        sym.is_some(),
        "expected Method 'SayHello'; got: {:?}",
        r.symbols
    );
}
...
fn cov_rpc_emits_type_refs_for_request_and_response() {
    ...
    let has_req = r
        .refs
        .iter()
        .any(|rf| rf.kind == EdgeKind::TypeRef && rf.target_name == "HelloRequest");
    let has_resp = r
        .refs
        .iter()
        .any(|rf| rf.kind == EdgeKind::TypeRef && rf.target_name == "HelloReply");
    assert!(
        has_req,
        "expected TypeRef to 'HelloRequest'; got: {:?}",
        r.refs
    );
    assert!(
        has_resp,
        "expected TypeRef to 'HelloReply'; got: {:?}",
        r.refs
    );
}
...
fn cov_enum_emits_enum() {
    ...
    let sym = r
        .symbols
        .iter()
        .find(|s| s.name == "OrderStatus" && s.kind == SymbolKind::Enum);
    assert!(
        sym.is_some(),
        "expected Enum 'OrderStatus'; got: {:?}",
        r.symbols
    );
}
...
fn cov_field_primitive_emits_field_no_type_ref() {
    ...
    let primitive_ref = r
        .refs
        .iter()
        .any(|rf| rf.kind == EdgeKind::TypeRef && rf.target_name == "string");
    assert!(
        !primitive_ref,
        "should not emit TypeRef for primitive 'string'; got: {:?}",
        r.refs
    );
}
...
fn cov_field_message_type_emits_type_ref() {
    ...
    let field = r
        .symbols
        .iter()
        .find(|s| s.name == "shipping_address" && s.kind == SymbolKind::Field);
    assert!(
        field.is_some(),
        "expected Field 'shipping_address'; got: {:?}",
        r.symbols
    );
    let has_ref = r
        .refs
        .iter()
        .any(|rf| rf.kind == EdgeKind::TypeRef && rf.target_name == "Address");
    assert!(
        has_ref,
        "expected TypeRef to 'Address' from message field; got: {:?}",
        r.refs
    );
}
...
fn cov_map_field_emits_field() {
    ...
    let sym = r
        .symbols
        .iter()
        .find(|s| s.name == "labels" && s.kind == SymbolKind::Field);
    assert!(
        sym.is_some(),
        "expected Field 'labels' from map_field; got: {:?}",
        r.symbols
    );
}
...
fn cov_map_field_message_value_emits_type_ref() {
    ...
    let has_ref = r
        .refs
        .iter()
        .any(|rf| rf.kind == EdgeKind::TypeRef && rf.target_name == "Service");
    assert!(
        has_ref,
        "expected TypeRef to 'Service' from map_field value type; got: {:?}",
        r.refs
    );
}
...
fn cov_package_emits_namespace() {
    ...
    let sym = r
        .symbols
        .iter()
        .find(|s| s.kind == SymbolKind::Namespace && s.name.contains("com"));
    assert!(
        sym.is_some(),
        "expected Namespace for package declaration; got: {:?}",
        r.symbols
    );
}
...
fn cov_import_emits_imports_edge() {
    ...
    let has_import = r
        .refs
        .iter()
        .any(|rf| rf.kind == EdgeKind::Imports && rf.target_name.contains("timestamp"));
    assert!(
        has_import,
        "expected Imports edge for import statement; got: {:?}",
        r.refs
    );
}
...
fn cov_oneof_emits_field_children() {
    ...
    let oneof = r
        .symbols
        .iter()
        .find(|s| s.name == "payload" && s.kind == SymbolKind::Field);
    assert!(
        oneof.is_some(),
        "expected Field 'payload' for oneof scope; got: {:?}",
        r.symbols
    );
    // oneof fields
    let text_field = r
        .symbols
        .iter()
        .find(|s| s.name == "text" && s.kind == SymbolKind::Field);
    assert!(
        text_field.is_some(),
        "expected Field 'text' inside oneof; got: {:?}",
        r.symbols
    );
}
...
fn cov_extend_emits_class_and_type_ref() {
    ...
    let sym = r
        .symbols
        .iter()
        .find(|s| s.kind == SymbolKind::Class && s.name.contains("extend"));
    assert!(
        sym.is_some(),
        "expected Class for extend block; got: {:?}",
        r.symbols
    );
    let has_ref = r
        .refs
        .iter()
        .any(|rf| rf.kind == EdgeKind::TypeRef && rf.target_name.contains("FieldOptions"));
    assert!(
        has_ref,
        "expected TypeRef for extended type; got: {:?}",
        r.refs
    );
}
...
fn cov_nested_message_emits_struct() {
    ...
    let outer = r
        .symbols
        .iter()
        .find(|s| s.name == "Outer" && s.kind == SymbolKind::Struct);
    assert!(
        outer.is_some(),
        "expected Struct 'Outer'; got: {:?}",
        r.symbols
    );
    let inner = r
        .symbols
    ...
    assert!(
        inner.is_some(),
        "expected nested Struct 'Inner'; got: {:?}",
        r.symbols
    );
}
...
fn cov_top_level_message_qualified_with_package() {
    ...
    let sym = r
        .symbols
        .iter()
        .find(|s| s.name == "KeyValue" && s.kind == SymbolKind::Struct);
    assert!(
        sym.is_some(),
        "expected Struct 'KeyValue'; got: {:?}",
...
fn cov_service_qualified_with_package() {
    ...
    let sym = r
        .symbols
        .iter()
        .find(|s| s.name == "Greeter" && s.kind == SymbolKind::Interface);
    assert!(
        sym.is_some(),
        "expected Interface 'Greeter'; got: {:?}",
        r.symbols.iter().map(|s| &s.name).collect::<Vec<_>>()
    );
...
fn cov_message_without_package_keeps_bare_qname() {
    ...
    let sym = r
        .symbols
        .iter()
        .find(|s| s.name == "Standalone" && s.kind == SymbolKind::Struct);
    assert!(sym.is_some());
