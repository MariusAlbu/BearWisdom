//! Kotlin multiline-string content detection (E19).
//!
//! Walks the Kotlin AST for `string_literal` nodes whose raw text starts
//! with `"""` (Kotlin raw strings — equivalent to Java text blocks).
...
fn walk(node: &Node, source: &str, regions: &mut Vec<EmbeddedRegion>) {
    let kind = node.kind();
    if kind.contains("string_literal") || kind == "multi_line_string_literal" {
...
mod tests {
    ...
    #[test]
    fn kotlin_single_quote_string_ignored() {
        let src = "fun q() { val s = \"hi\" }";
        let regions = detect_regions(src);
