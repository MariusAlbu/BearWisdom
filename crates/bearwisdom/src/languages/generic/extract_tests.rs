...
fn python_imports() {
    ...
    let result = must_extract(src, "python");
    assert!(!result.refs.is_empty(), "expected at least one import ref");
...
"#;
    ...
    assert!(
        ...
        "expected Point struct, got {names:?}"
    );
    assert!(
        names.contains(&"NewPoint"),
        "expected NewPoint, got {names:?}"
    );
}
...
"#;
    ...
    assert!(
        has_imports,
        "expected at least one Imports ref for Go imports"
    );
}
...
"#;
    ...
    assert!(
        ...
        "expected Shape trait, got {names:?}"
    );
    assert!(
        names.contains(&"distance"),
        "expected distance fn, got {names:?}"
    );
}
...
fn rust_use_declarations() {
    ...
    assert!(
        !imports.is_empty(),
        "expected Imports refs for use declarations"
    );
}
...
fn empty_source_does_not_panic() {
    for lang in &["python", "java", "go", "rust", "ruby", "cpp", "c"] {
        ...
        assert!(
            ...
            "{lang}: empty src should have no symbols"
        );
    }
}
