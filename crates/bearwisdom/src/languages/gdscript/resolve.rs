...
impl LanguageResolver for GDScriptResolver {
    ...
    fn resolve(
        ...
        // Step 1: Scope chain walk (inner class → outer class → script).
        ...
        for sym in lookup.in_file(&file_ctx.file_path) {
            if sym.name == *target && builtins::kind_compatible(edge_kind, &sym.kind) {
                return Some(Resolution {
                    target_symbol_id: sym.id,
                    confidence: 1.0,
                    strategy: "gdscript_same_file",
                });
            }
        }

        // Step 3: Project-wide name lookup (global class_name registrations).
        for sym in lookup.by_name(target) {
            if builtins::kind_compatible(edge_kind, &sym.kind) {
                return Some(Resolution {
                    target_symbol_id: sym.id,
                    confidence: 0.85,
                    strategy: "gdscript_by_name",
                });
            }
        }
