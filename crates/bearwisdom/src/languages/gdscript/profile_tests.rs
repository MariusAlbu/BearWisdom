...
// ---------------------------------------------------------------------------
// Full-ladder bind tests through resolve_all_with_profile(&GDSCRIPT_PROFILE).
// ---------------------------------------------------------------------------
...
fn resolve_ref(file_path: &str, target: &str, kind: EdgeKind, all: &[&ParsedFile]) -> Option<Resolution> {
    ...
    .resolve_all_with_profile(&GDSCRIPT_PROFILE)
}

#[test]
fn gdscript_bare_call_binds_internal_over_external() {
    // `take_damage` is the project's own global function (defined in two `.gd`
    // scripts) plus a same-named ext: engine stub. A bare cross-file call binds
    // an INTERNAL definition first-match; the external stub loses.
    let a = make_file(
        "scripts/player.gd",
        vec![make_sym("take_damage", SymbolKind::Function)],
        vec![],
    );
    let b = make_file(
        "scripts/enemy.gd",
    ...
    let (id_a, id_b, ext_id) = {
        ...
        (
            sym_id(&id_map, "scripts/player.gd", "take_damage"),
            sym_id(&id_map, "scripts/enemy.gd", "take_damage"),
            sym_id(&id_map, "ext:gdscript:godot/node.gd", "take_damage"),
        )
    };
    let res = resolve_ref("scripts/main.gd", "take_damage", EdgeKind::Calls, &[&a, &b, &ext])
        .expect("bare GDScript call binds an internal function");
    assert_eq!(res.strategy, "default_namespaceless_global");
    assert_ne!(
        res.target_symbol_id, ext_id,
        "must not bind the ext engine stub"
    );
    assert!(
        res.target_symbol_id == id_a || res.target_symbol_id == id_b,
        "binds an internal take_damage (got {})",
        res.target_symbol_id
    );
}

#[test]
fn gdscript_external_only_name_stays_unresolved() {
    // `queue_free` (Godot engine) is owned ONLY by an external file — no
    // project definition. The internal-only rung declines, leaving it for
    // external classification.
    let ext = make_file(
        "ext:gdscript:godot/node.gd",
        vec![make_sym("queue_free", SymbolKind::Function)],
        vec![],
    );
    let res = resolve_ref("scripts/main.gd", "queue_free", EdgeKind::Calls, &[&ext]);
    assert!(
        res.is_none(),
