// =============================================================================
// languages/vue/extract.rs  —  Vue SFC extractor
//
// What we extract
// ---------------
// SYMBOLS:
//   Class     — the component itself (name = filename stem, line 0)
//
// REFERENCES (source_symbol_index = 0, the component symbol):
//   Calls     — PascalCase tags in <template> (component usages)
//   Calls     — kebab-case custom element tags (normalized to PascalCase)
//   Calls     — event handler directives: @click="handler" / v-on:click="handler"
//
// Grammar: tree-sitter-html 0.23 (used as structural fallback).
// The <script> block content is recorded but not parsed here — the JS/TS
// extractor handles it separately when the indexer processes injection points.
// =============================================================================

use crate::types::{EdgeKind, ExtractedRef, ExtractedSymbol, SymbolKind, Visibility};
use tree_sitter::{Node, Parser};

/// HTML-like tag names that are standard HTML elements, not Vue components.
/// PascalCase check already filters most false positives; this set covers
/// the small number of uppercase-starting HTML5 tags to be safe.
const BUILTIN_HTML_TAGS: &[&str] = &["DOCTYPE", "CDATA"];

pub fn extract(source: &str, file_path: &str) -> super::ExtractionResult {
    let language: tree_sitter::Language = tree_sitter_html::LANGUAGE.into();
    let mut parser = Parser::new();
    parser
        .set_language(&language)
        .expect("Failed to load HTML grammar for Vue");

    let tree = match parser.parse(source, None) {
        Some(t) => t,
        None => return super::ExtractionResult::new(vec![], vec![], true),
    };

    let has_errors = tree.root_node().has_error();
    let mut symbols: Vec<ExtractedSymbol> = Vec::new();
    let mut refs: Vec<ExtractedRef> = Vec::new();

    // Infer component name from filename stem. Kebab-case filenames are
    // converted to PascalCase: `my-button.vue` → `MyButton`. This matches
    // what users write in import statements
    // (`import MyButton from './my-button.vue'`) — without the conversion,
    // the symbol's name field is `my-button` and the resolver can't match
    // the imported alias against any indexed symbol in the file.
    let stem = file_stem(file_path);
    let component_name = if stem.contains('-') {
        kebab_to_pascal(&stem)
    } else {
        stem.clone()
    };
    symbols.push(ExtractedSymbol {
        name: component_name.clone(),
        qualified_name: component_name.clone(),
        kind: SymbolKind::Class,
        visibility: Some(Visibility::Public),
        start_line: 0,
        end_line: tree.root_node().end_position().row as u32,
        start_col: 0,
        end_col: 0,
        signature: Some(format!("// Vue component: {component_name}")),
        doc_comment: None,
        scope_path: None,
        parent_index: None,
    });

    // Walk the document to find template elements and extract component usages.
    let root = tree.root_node();
    visit_document(&root, source, &mut refs);

    super::ExtractionResult::new(symbols, refs, has_errors)
}

// ---------------------------------------------------------------------------
// Document traversal
// ---------------------------------------------------------------------------

fn visit_document(node: &Node, src: &str, refs: &mut Vec<ExtractedRef>) {
    let mut cursor = node.walk();
    for child in node.children(&mut cursor) {
        match child.kind() {
            "element" => {
                // Check if this is the <template> top-level block
                let tag = element_tag_name(&child, src);
                if tag.eq_ignore_ascii_case("template") {
                    // Walk the template body for component usages
                    visit_template(&child, src, refs);
                } else {
                    // Could be a root-level element in template-only files
                    visit_template(&child, src, refs);
                }
            }
            _ => visit_document(&child, src, refs),
        }
    }
}

fn visit_template(node: &Node, src: &str, refs: &mut Vec<ExtractedRef>) {
    let mut cursor = node.walk();
    for child in node.children(&mut cursor) {
        match child.kind() {
            "element" => {
                process_element(&child, src, refs);
                visit_template(&child, src, refs);
            }
            "self_closing_element" => {
                process_element(&child, src, refs);
            }
            _ => visit_template(&child, src, refs),
        }
    }
}

// ---------------------------------------------------------------------------
// Element processing
// ---------------------------------------------------------------------------

fn process_element(node: &Node, src: &str, refs: &mut Vec<ExtractedRef>) {
    let tag = element_tag_name(node, src);

    // Component usages: PascalCase or kebab-case with hyphens
    if let Some(component_name) = as_component_name(&tag) {
        refs.push(ExtractedRef {
            source_symbol_index: 0,
            target_name: component_name,
            kind: EdgeKind::Calls,
            line: node.start_position().row as u32,
            module: None,
            chain: None,
            byte_offset: 0,
                    namespace_segments: Vec::new(),
});
    }

    // Scan attributes for event handler directives
    extract_event_handlers(node, src, refs);
}

// ---------------------------------------------------------------------------
// Event handler directives
// ---------------------------------------------------------------------------

fn extract_event_handlers(node: &Node, src: &str, refs: &mut Vec<ExtractedRef>) {
    let mut cursor = node.walk();
    for child in node.children(&mut cursor) {
        match child.kind() {
            "start_tag" | "self_closing_tag" => {
                let mut ac = child.walk();
                for attr in child.children(&mut ac) {
                    if attr.kind() == "attribute" {
                        try_extract_event_handler(&attr, src, refs);
                    }
                }
            }
            _ => {}
        }
    }
}

fn try_extract_event_handler(node: &Node, src: &str, refs: &mut Vec<ExtractedRef>) {
    // attribute has attribute_name and (optionally) attribute_value / quoted_attribute_value
    let attr_name = node
        .child_by_field_name("name")
        .or_else(|| find_child_of_kind(node, "attribute_name"))
        .map(|n| node_text(n, src))
        .unwrap_or_default();

    // Match @event="handler" or v-on:event="handler"
    let is_event = attr_name.starts_with('@') || attr_name.starts_with("v-on:");
    if !is_event {
        return;
    }

    let handler = node
        .child_by_field_name("value")
        .or_else(|| find_child_of_kind(node, "quoted_attribute_value"))
        .or_else(|| find_child_of_kind(node, "attribute_value"))
        .map(|n| {
            let raw = node_text(n, src);
            // Strip surrounding quotes
            raw.trim_matches('"').trim_matches('\'').to_string()
        })
        .unwrap_or_default();

    // Strip argument: "handler($event)" → "handler", "method()" → "method"
    let handler = handler
        .split('(')
        .next()
        .unwrap_or(&handler)
        .trim()
        .to_string();

    if handler.is_empty() || handler.contains(' ') || handler.contains('{') {
        return; // inline expression, skip
    }

    refs.push(ExtractedRef {
        source_symbol_index: 0,
        target_name: handler,
        kind: EdgeKind::Calls,
        line: node.start_position().row as u32,
        module: None,
        chain: None,
        byte_offset: 0,
            namespace_segments: Vec::new(),
});
}

// ---------------------------------------------------------------------------
// Component name detection
// ---------------------------------------------------------------------------

/// Returns `Some(PascalCase name)` if `tag` looks like a Vue component reference.
fn as_component_name(tag: &str) -> Option<String> {
    if tag.is_empty() {
        return None;
    }
    // Skip known HTML-only uppercase fragments
    if BUILTIN_HTML_TAGS.contains(&tag) {
        return None;
    }
    // PascalCase: starts with uppercase
    if tag.chars().next().map_or(false, |c| c.is_uppercase()) {
        return Some(tag.to_string());
    }
    // Kebab-case: contains at least one hyphen → convert to PascalCase
    if tag.contains('-') {
        let pascal = kebab_to_pascal(tag);
        return Some(pascal);
    }
    None
}

fn kebab_to_pascal(s: &str) -> String {
    s.split('-')
        .map(|part| {
            let mut c = part.chars();
            match c.next() {
                None => String::new(),
                Some(first) => first.to_uppercase().collect::<String>() + c.as_str(),
            }
        })
        .collect()
}

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

fn element_tag_name(node: &Node, src: &str) -> String {
    // element → start_tag → tag_name
    // self_closing_element → tag_name (direct child)
    let mut cursor = node.walk();
    for child in node.children(&mut cursor) {
        match child.kind() {
            "start_tag" | "self_closing_tag" => {
                if let Some(tag) = find_child_of_kind(&child, "tag_name") {
                    return node_text(tag, src);
                }
            }
            "tag_name" => return node_text(child, src),
            _ => {}
        }
    }
    String::new()
}

fn find_child_of_kind<'a>(node: &'a Node<'a>, kind: &str) -> Option<Node<'a>> {
    let mut cursor = node.walk();
    for child in node.children(&mut cursor) {
        if child.kind() == kind {
            return Some(child);
        }
    }
    None
}

fn node_text(node: Node, src: &str) -> String {
    src[node.start_byte()..node.end_byte()].to_string()
}

fn file_stem(path: &str) -> String {
    let file = path.rsplit('/').next().unwrap_or(path);
    let file = file.rsplit('\\').next().unwrap_or(file);
    // Strip extension
    if let Some(dot) = file.rfind('.') {
        file[..dot].to_string()
    } else {
        file.to_string()
    }
}
