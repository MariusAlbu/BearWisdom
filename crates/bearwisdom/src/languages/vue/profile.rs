...
pub const VUE_PROFILE: LanguageProfile = LanguageProfile {
    ...
    // `this` in an Options-API method body is the Vue component instance type,
    // declared in the installed Vue package — no user-source `extends` for the
    // extractor to capture. Discover it structurally by its canonical member
    // set (`$emit` + `$nextTick` + `$forceUpdate`), which spans Vue 2's
    // `vue.Vue` and Vue 3's `ComponentPublicInstance` without version branching.
    self_receiver_discovery:
        crate::type_checker::profile::language_profile::SelfReceiverDiscovery::CanonicalMembers {
            seed: "$emit",
