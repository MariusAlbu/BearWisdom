...
fn is_python_builtin_drains_bare_call_names() {
    ...
    for name in [
        ...
        assert!(
            ...
            "{name} should drain as a bare call"
        );
    }
}
...
fn is_python_builtin_drains_bare_type_ref_names() {
    ...
    for name in [
        ...
        assert!(
            is_python_builtin(name),
            "{name} should drain as a bare type_ref"
        );
    }
}
...
fn is_python_builtin_declines_generic_type_params() {
    ...
    for name in ["T", "U", "K", "V", "E", "R", "S"] {
        assert!(
            !is_python_builtin(name),
            "{name} is a generic param, not a builtin"
        );
    }
}
...
fn is_python_builtin_declines_stdlib_with_source() {
    ...
    for name in [
        ...
        assert!(
            !is_python_builtin(name),
            "{name} has real stdlib source, must not drain"
        );
    }
}
...
fn is_python_builtin_declines_project_names() {
    for name in [
        ...
        assert!(
            !is_python_builtin(name),
            "{name} is a project name, not a builtin"
        );
    }
}
