...
fn cov_function_definition_produces_function_symbol() {
    ...
    assert!(
        sym.is_some(),
        "expected Function symbol 'bar', got: {:?}",
        r.symbols
    );
...
fn cov_decorated_definition_produces_symbol() {
    ...
    assert!(
        sym.is_some(),
        "expected symbol from decorated_definition, got: {:?}",
        r.symbols
    );
}
...
fn cov_assignment_produces_variable_symbol() {
    ...
    assert!(
        sym.is_some(),
        "expected Variable symbol 'MAX_SIZE', got: {:?}",
        r.symbols
    );
}
...
fn cov_call_produces_calls_ref() {
    ...
    assert!(
        calls.contains(&"foo"),
        "expected Calls ref for foo(), got: {calls:?}"
    );
}
...
fn cov_import_statement_produces_imports_ref() {
    ...
    assert!(
        imports.contains(&"os"),
        "expected Imports ref for 'os', got: {imports:?}"
    );
}
...
fn cov_import_from_statement_produces_imports_ref() {
    ...
    assert!(
        ...
        "expected Imports ref for 'path', got: {imports:?}"
    );
}
...
fn cov_class_definition_with_superclass_produces_inherits_ref() {
    ...
    let r = extract::extract(src);
    let refs: Vec<(&str, EdgeKind)> = r
        .refs
        .iter()
        .map(|rf| (rf.target_name.as_str(), rf.kind))
        .collect();
    assert!(
        refs.iter().any(|(name, kind)| *name == "Animal"
            && (*kind == EdgeKind::TypeRef || *kind == EdgeKind::Inherits)),
...
fn cov_function_definition_inside_class_produces_method_symbol() {
    ...
    assert!(
        sym.is_some(),
        "expected Method symbol 'handle', got: {:?}",
        r.symbols
    );
...
fn cov_init_method_produces_constructor_symbol() {
    ...
    assert!(
        sym.is_some(),
        "expected Constructor symbol '__init__', got: {:?}",
        r.symbols
    );
