...
fn library_with_explicit_py_extension_resolves_directly() {
    ...
    assert!(
        entry
            .iter()
            .any(|l| l.py_file_path == "atest/testdata/keywords/KeywordDecorator.py"),
        "got {entry:?}"
    );
}
...
fn dotted_library_name_resolves_via_last_segment() {
    let parsed = vec![
        pf("robot/util/MyLib.py", Vec::new()),
        pf("tests/foo.robot", vec![import_ref("robot.util.MyLib")]),
    ];
    let map = build_robot_library_map(&parsed);
...
fn resource_cycle_does_not_loop_forever() {
    ...
    let parsed = vec![
        pf("a.robot", vec![import_ref("b.robot")]),
        pf("b.robot", vec![import_ref("a.robot")]),
    ];
...
fn relative_resource_path_resolves_to_indexed_basename() {
    ...
    assert!(
        entry.iter().any(|l| l.py_file_path == "py/Lib.py"),
...
fn library_in_same_dir_wins_over_distant_match() {
    ...
    let entry = map
        ...
        .expect("must resolve");
    assert_eq!(
        entry[0].py_file_path, "atest/resources/TestCheckerLibrary.py",
...
fn no_builtin_py_means_no_auto_import() {
    ...
    let parsed = vec![pf("tests/no_imports.robot", Vec::new())];
    let map = build_robot_library_map(&parsed);
    assert!(
...
fn basename_map_resolves_resource_imports_to_full_paths() {
    ...
    assert_eq!(
        map.get("atest_resource.robot").map(|v| v.as_slice()),
