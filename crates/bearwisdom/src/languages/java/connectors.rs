// =============================================================================
// languages/java/connectors.rs — Spring route discovery
//
// Scans indexed Java files for @GetMapping/@PostMapping/etc. and
// @RequestMapping annotations, writing rows to the `routes` table. The
// routes-table → FlowEmission bridge in resolve/mod.rs converts those
// rows into Consumer HttpCall flow_edges.
//
// DI bindings (stereotype-annotated classes) now flow through the Java
// resolver's `detect_flow_emission` arm as resolver-side FlowEmissions.
// =============================================================================
...
/// Spring controller scan: class `@RequestMapping` prefix + method
/// `@GetMapping` etc. → flat `(method, path)` rows written to the `routes`
/// table. The routes-table → FlowEmission bridge in resolve/mod.rs handles
/// downstream flow_edges emission.
///
/// Returns the count of routes written to the `routes` table.
pub fn discover_spring_routes(conn: &Connection, project_root: &Path) -> u32 {
    let routes = match find_spring_routes(conn, project_root) {
        Ok(r) => r,
        Err(e) => {
            tracing::warn!("Spring route detection failed: {e}");
            return 0;
        }
    };

    if let Err(e) = write_routes(conn, &routes) {
        tracing::warn!("Spring route insert failed: {e}");
        return 0;
    }
    routes.len() as u32
}
