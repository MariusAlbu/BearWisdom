...
fn coverage_constant_declaration() {
    ...
    assert!(
        s.iter()
            .any(|s| s.name == "MAX" && s.kind == SymbolKind::Field),
...
fn coverage_package_declaration() {
    ...
    assert_eq!(
        ns.qualified_name, "com.example.service",
...
fn coverage_method_invocation() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "println" && r.kind == EdgeKind::Calls),
...
fn coverage_object_creation_expression() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "ArrayList" && r.kind == EdgeKind::Instantiates),
...
fn coverage_import_declaration() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "List" && r.kind == EdgeKind::Imports),
...
fn coverage_annotation_marker() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Service" && r.kind == EdgeKind::TypeRef),
...
fn coverage_annotation_with_args() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "GetMapping" && r.kind == EdgeKind::TypeRef),
...
fn coverage_superclass() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "BaseService" && r.kind == EdgeKind::Inherits),
...
fn coverage_super_interfaces() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Runnable" && r.kind == EdgeKind::Implements),
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Serializable" && r.kind == EdgeKind::Implements),
...
fn coverage_extends_interfaces() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Repository" && r.kind == EdgeKind::Implements),
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "ReadOnly" && r.kind == EdgeKind::Implements),
...
fn coverage_field_declaration_type_ref() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "List" && r.kind == EdgeKind::TypeRef),
...
fn coverage_field_declaration_annotation() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Autowired" && r.kind == EdgeKind::TypeRef),
...
fn coverage_type_arguments_in_method_call() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "emptyList" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        syms.iter()
            .any(|(n, k)| *n == "count" && *k == crate::types::SymbolKind::Field),
        "expected Field 'count' from anon class in constructor args; symbols: {:?}",
        syms
    );
    assert!(
        syms.iter()
            .any(|(n, k)| *n == "onEvent" && *k == crate::types::SymbolKind::Method),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "process" && r.kind == EdgeKind::Calls),
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "isValid" && r.kind == EdgeKind::Calls),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "getA" && r.kind == EdgeKind::Calls),
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "getB" && r.kind == EdgeKind::Calls),
...
fn coverage_annotation_type_element_method() {
    ...
    assert!(
        s.iter()
            .any(|s| s.name == "timeout" && s.kind == SymbolKind::Method),
...
fn coverage_compact_constructor_as_constructor() {
    ...
    assert!(
        s.iter()
            .any(|s| s.name == "Point" && s.kind == SymbolKind::Constructor),
...
fn coverage_explicit_constructor_invocation_super() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "super" && r.kind == EdgeKind::Calls),
...
fn debug_measure_coverage_java() {
    ...
    let project_path = match project_path {
        Some(p) => p,
        None => {
            eprintln!("No Java test project found");
            return;
        }
    };
