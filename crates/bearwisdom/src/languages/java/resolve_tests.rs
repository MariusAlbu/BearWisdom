...
fn lombok_synthesized_methods_carry_return_types_in_index() {
    ...
    assert_eq!(
        index.return_type_name("User.builder"),
        Some("User.UserBuilder")
    );
    assert_eq!(
        index.return_type_name("User.UserBuilder.name"),
        Some("User.UserBuilder")
    );
    assert_eq!(
        index.return_type_name("User.UserBuilder.build"),
        Some("User")
    );
...
fn test_java_http_no_emit_when_url_is_variable() {
    ...
    let chain = make_chain(&["restTemplate", "getForObject"]);
    let call_args = vec![
        CallArg::Ident("url".to_string()),
        CallArg::Ident("Object".to_string()),
    ];
    assert!(detect_java_http_chain_emission(&chain, &call_args).is_none());
}
...
fn test_java_jdbc_template_query_select() {
    ...
    let args = vec![
        CallArg::StringLit("SELECT id FROM users WHERE x = ?".to_string()),
        CallArg::Other,
    ];
