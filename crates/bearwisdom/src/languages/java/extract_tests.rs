...
"#;
    ...
    assert!(symbols
        .iter()
        .any(|s| s.name == "delete" && s.kind == SymbolKind::Method));
}
...
"#;
    ...
    assert!(
        list_import.is_some(),
        "Missing List import; imports: {:?}",
        imports.iter().map(|i| &i.target_name).collect::<Vec<_>>()
    );
    assert_eq!(
        list_import.unwrap().module.as_deref(),
        Some("java.util.List")
    );
...
"#;
    ...
    assert!(
        ...
        "Missing doSomething; calls: {names:?}"
    );
}
...
"#;
    ...
    let parameterized = symbols
        .iter()
        .find(|s| s.name == "addsManyNumbers")
        .unwrap();
    assert_eq!(
        parameterized.kind,
        SymbolKind::Test,
        "addsManyNumbers should be Test"
    );
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Cloneable" && r.kind == EdgeKind::Implements),
...
"#;
    let r = refs(src);
    let instantiations: Vec<_> = r
        .iter()
        .filter(|r| r.kind == EdgeKind::Instantiates)
        .collect();
    let names: Vec<&str> = instantiations
        .iter()
    ...
    assert!(
        names.contains(&"ArrayList"),
        "Missing ArrayList; got: {names:?}"
    );
}
...
"#;
    ...
    assert!(
        s.iter()
            .any(|s| s.name == "admin" && s.kind == SymbolKind::Variable),
...
"#;
    ...
    assert!(
        ...
        "Missing 'getName' inside lambda: {calls:?}"
    );
...
"#;
    ...
    assert!(
        s.iter()
            .any(|s| s.name == "u" && s.kind == SymbolKind::Parameter),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Comparable" && r.kind == EdgeKind::Implements),
...
"#;
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "IOException" && r.kind == EdgeKind::TypeRef),
