...
"#;
    ...
    assert!(
        props.contains(&"name"),
        "expected 'name' Property from promotion: {props:?}"
    );
    assert!(
        props.contains(&"age"),
        "expected 'age' Property from promotion: {props:?}"
    );
}
...
fn include_produces_imports_edge() {
    ...
    assert!(
        imp.is_some(),
        "expected Imports edge from include, refs: {:?}",
        r.refs
    );
    let imp = imp.unwrap();
    assert_eq!(
        imp.target_name, "utils",
        "expected target 'utils': {}",
        imp.target_name
    );
}
...
fn static_call_emits_type_access_chain() {
    ...
    assert_eq!(
        chain.segments.len(),
        2,
        "chain must have 2 segments: [TypeAccess(File), Property(whereIn)]"
    );
    ...
    assert_eq!(
        ...
        "chain root kind must be TypeAccess for static calls"
    );
