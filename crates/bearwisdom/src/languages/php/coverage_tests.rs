...
fn cov_interface_declaration_produces_interface_symbol() {
    ...
    assert!(
        sym.is_some(),
        "expected Interface symbol 'Drawable', got: {:?}",
        r.symbols
    );
...
fn cov_trait_declaration_produces_symbol() {
    ...
    assert!(
        sym.is_some(),
        "expected symbol for trait 'Timestampable', got: {:?}",
        r.symbols
    );
}
...
fn cov_function_definition_produces_function_symbol() {
    ...
    assert!(
        sym.is_some(),
        "expected Function symbol 'compute', got: {:?}",
        r.symbols
    );
...
fn cov_method_declaration_produces_method_symbol() {
    ...
    assert!(
        sym.is_some(),
        "expected Method symbol 'run', got: {:?}",
        r.symbols
    );
...
fn cov_const_declaration_produces_constant_symbol() {
    ...
    assert!(
        sym.is_some(),
        "expected symbol for class const 'MAX', got: {:?}",
        r.symbols
    );
}
...
fn cov_namespace_definition_produces_namespace_symbol() {
    ...
    let sym = r
        .symbols
        .iter()
        .find(|s| s.name.contains("Models") || s.name.contains("App"));
    assert!(
        sym.is_some(),
        "expected Namespace symbol from namespace_definition, got: {:?}",
        r.symbols
    );
...
fn cov_function_call_expression_produces_calls_ref() {
    ...
    let calls: Vec<&str> = r
        ...
        .collect();
    assert!(
        calls.contains(&"strlen"),
        "expected Calls ref for strlen(), got: {calls:?}"
    );
}
...
fn cov_member_call_expression_produces_calls_ref() {
    ...
    assert!(
        ...
        "expected Calls ref for member_call_expression, got: {calls:?}"
    );
}
...
fn cov_nullsafe_member_call_produces_calls_ref() {
    ...
    assert!(
        ...
        "expected Calls ref for nullsafe_member_call_expression, got: {calls:?}"
    );
}
...
fn cov_scoped_call_expression_produces_calls_ref() {
    ...
    assert!(
        calls.contains(&"log"),
        "expected Calls ref for scoped_call_expression, got: {calls:?}"
    );
}
...
fn cov_object_creation_expression_produces_instantiates_ref() {
    ...
    let inst: Vec<&str> = r
        ...
        .collect();
    assert!(
        inst.contains(&"DateTime"),
        "expected Instantiates ref for new DateTime(), got: {inst:?}"
    );
}
...
fn cov_namespace_use_declaration_produces_imports_ref() {
    ...
    assert!(
        ...
        "expected Imports ref for 'User', got: {imports:?}"
    );
}
...
fn cov_use_declaration_trait_produces_implements_ref() {
    ...
    let impls: Vec<&str> = r
        ...
        .collect();
    assert!(
        impls.contains(&"MyTrait"),
        "expected Implements ref for use_declaration (trait), got: {impls:?}"
    );
}
...
fn cov_base_clause_produces_inherits_ref() {
    ...
    assert!(
        ...
        "expected Inherits ref from base_clause, got: {inherits:?}"
    );
}
...
fn cov_class_interface_clause_produces_implements_ref() {
    ...
    let impls: Vec<&str> = r
        ...
        .collect();
    assert!(
        impls.contains(&"Drawable"),
        "expected Implements ref for Drawable, got: {impls:?}"
    );
    assert!(
        impls.contains(&"Serializable"),
        "expected Implements ref for Serializable, got: {impls:?}"
    );
}
...
fn cov_named_type_in_param_produces_type_ref() {
    ...
    assert!(
        ...
        "expected TypeRef for 'Request' named_type, got: {type_refs:?}"
    );
}
...
fn cov_intersection_type_in_param_produces_type_refs() {
    let src =
        "<?php class Handler { public function handle(Countable&Stringable $val): void {} }\n";
    let r = extract::extract(src);
    let type_refs: Vec<&str> = r
        .refs
        .iter()
...
fn cov_method_declaration_construct_produces_constructor_symbol() {
    ...
    assert!(
        sym.is_some(),
        "expected symbol for __construct, got: {:?}",
        r.symbols
    );
    assert_eq!(
        sym.unwrap().kind,
...
fn cov_enum_with_implements_produces_implements_ref() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.kind == EdgeKind::Implements && rf.target_name == "HasLabel"),
