...
fn symbol_paragraph() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "MAIN-PARA" && s.kind == SymbolKind::Function),
...
fn symbol_section() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "INIT-SECTION" && s.kind == SymbolKind::Function),
...
fn symbol_data_description() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "WS-COUNTER" && s.kind == SymbolKind::Variable),
...
fn symbol_perform_statement_produces_ref() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "WORK-PARA" && rf.kind == EdgeKind::Calls),
...
fn ref_call_statement() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "EXTPROG" && rf.kind == EdgeKind::Calls),
...
fn ref_copy_statement() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "MYLIB" && rf.kind == EdgeKind::Imports),
...
fn symbol_data_description_level_77() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "WS-FLAG" && s.kind == SymbolKind::Variable),
...
fn symbol_data_description_subordinate_field() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "WS-NAME" && s.kind == SymbolKind::Field),
...
fn ref_call_statement_also_imports() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "SUBPROG" && rf.kind == EdgeKind::Imports),
...
fn ref_copy_statement_in_data_division() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "DATALIB" && rf.kind == EdgeKind::Imports),
...
fn symbol_program_definition() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "MYPROGRAM" && s.kind == SymbolKind::Namespace),
...
fn ref_goto_statement() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "END-PARA" && rf.kind == EdgeKind::Calls),
