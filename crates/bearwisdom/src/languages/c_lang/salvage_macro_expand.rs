...
pub(super) fn salvage_macro_expanded_decls(
    ...
    for inv in invocations {
        let Some(def) = catalog.by_name.get(&inv.name) else {
            continue;
        };
...
fn expand_one_pass(text: &str, catalog: &super::macro_catalog::MacroCatalog) -> String {
...
fn scan_known_macro_invocations(
    ...
    while i < bytes.len() {
        ...
        let Some((args_str, end)) = collect_balanced_parens(source, j) else {
            continue;
        };
        let args = split_macro_args(&args_str);
        if args.len() != def.args.len() {
...
fn try_emit_typedef(
    ...
    if matches!(name, "struct" | "union" | "enum") {
        return;
    }
    push_if_missing(
        symbols,
...
fn try_emit_function_decl(
    ...
    if !is_ident_byte(bytes[t - 1]) && bytes[t - 1] != b'*' {
        return;
    }
    push_if_missing(
        symbols,
