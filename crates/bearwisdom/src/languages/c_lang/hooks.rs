...
/// C/C++ `ChainConfig` for the unified `resolve_via_chain`. Chain refs route
/// through the structured `ChainWalker` under `C_LANG_PROFILE` in production;
/// this config anchors the differential chain-walker tests that pin C's
/// case-space (local-typed roots, `::`→`.` normalization, field-type
/// progression, typedef-dereference via alias expansion).
///
