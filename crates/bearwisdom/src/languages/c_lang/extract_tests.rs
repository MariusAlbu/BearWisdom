...
"#;
    ...
    assert!(r
        .symbols
        .iter()
        .any(|s| s.name == "NORTH" && s.kind == SymbolKind::EnumMember));
    assert!(r
        .symbols
        .iter()
        .any(|s| s.name == "WEST" && s.kind == SymbolKind::EnumMember));
}
...
"#;
    ...
    let has_ref = r
        .refs
        .iter()
        .any(|rf| rf.target_name == "MyAllocator" && rf.kind == EdgeKind::TypeRef);
    assert!(
        has_ref,
        "expected TypeRef to `MyAllocator` from default type param: {:?}",
        r.refs
    );
}
...
"#;
    ...
    // in the graph.
    let has_target_ref = r
        .refs
        .iter()
        .any(|rf| rf.kind == EdgeKind::TypeRef && rf.target_name == "DeriveColors");
    assert!(
        has_target_ref,
        "alias must emit TypeRef to its target namespace: {:?}",
        r.refs
    );
}
...
fn cpp_macro_prefixed_class_emits_real_class_symbol_when_isolated() {
    ...
    let bogus = r
        .symbols
        .iter()
        .find(|s| s.name == "Q_WIDGETS_EXPORT" && s.kind == SymbolKind::Class);
    assert!(
        bogus.is_none(),
        "Q_WIDGETS_EXPORT must not be emitted as a class symbol: {:?}",
        r.symbols
    );
    ...
    let inherits = r
        .refs
        .iter()
        .any(|rf| rf.kind == EdgeKind::Inherits && rf.target_name == "QDialog");
    assert!(
        inherits,
        "salvage must emit Inherits TypeRef for base class: {:?}",
        r.refs
    );
}
...
"#;
    ...
    let has_mytype = r
        .refs
        .iter()
        .any(|rf| rf.target_name == "MyType" && rf.kind == EdgeKind::TypeRef);
    assert!(
        has_mytype,
        "expected TypeRef to MyType from cast: {:?}",
        r.refs
    );
}
...
"#;
    ...
    let has_node = r
        .refs
        .iter()
        .any(|rf| rf.target_name == "Node" && rf.kind == EdgeKind::TypeRef);
    assert!(
        has_node,
        "expected TypeRef to Node from sizeof: {:?}",
        r.refs
    );
}
...
"#;
    let r = extract::extract(src, "cpp");
    let has_instantiates = r
        .refs
        .iter()
        .any(|rf| rf.target_name == "Widget" && rf.kind == EdgeKind::Instantiates);
    assert!(
        has_instantiates,
        "expected Instantiates to Widget: {:?}",
        r.refs
    );
    ...
    let has_typeref = r
        .refs
        .iter()
        .any(|rf| rf.target_name == "Widget" && rf.kind == EdgeKind::TypeRef);
...
"#;
    ...
    let has_cb = r
        .refs
        .iter()
        .any(|rf| rf.target_name == "Callback" && rf.kind == EdgeKind::TypeRef);
    assert!(
        has_cb,
        "expected TypeRef to Callback from lambda param: {:?}",
        r.refs
    );
}
...
"#;
    ...
    assert!(
        ...
        "missing do_work call: {calls:?}"
    );
    assert!(
        calls.contains(&"cleanup"),
        "missing cleanup call: {calls:?}"
    );
}
...
"#;
    let r = extract::extract(src, "cpp");
    let has_runtime_error = r
        .refs
        .iter()
        .any(|rf| rf.target_name == "runtime_error" && rf.kind == EdgeKind::TypeRef);
    assert!(
        has_runtime_error,
        "expected TypeRef to runtime_error from catch clause: {:?}",
        r.refs
    );
}
...
"#;
    ...
    let calls: Vec<&str> = r
        ...
        .collect();
    assert!(
        calls.contains(&"connect"),
        "missing connect call: {calls:?}"
    );
    assert!(
        calls.contains(&"log_exception"),
        "missing log_exception call: {calls:?}"
    );
    assert!(
        calls.contains(&"recover"),
        "missing recover call: {calls:?}"
    );
}
...
"#;
    ...
    let refs_names: Vec<&str> = r
        ...
        .collect();
    assert!(
        refs_names.contains(&"User"),
        "missing User TypeRef: {refs_names:?}"
    );
}
...
"#;
    ...
    let type_refs: Vec<&str> = r
        ...
        .collect();
    assert!(
        type_refs.contains(&"Session"),
        "missing Session TypeRef: {type_refs:?}"
    );
    assert!(
        type_refs.contains(&"UserContext"),
        "missing UserContext TypeRef: {type_refs:?}"
    );
}
...
"#;
    ...
    let type_refs: Vec<&str> = r
        ...
        .collect();
    assert!(
        !type_refs.contains(&"int"),
        "int should not produce TypeRef: {type_refs:?}"
    );
    assert!(
        !type_refs.contains(&"float"),
        "float should not produce TypeRef: {type_refs:?}"
    );
}
...
"#;
    ...
    let type_refs: Vec<&str> = r
        ...
        .collect();
    assert!(
        type_refs.contains(&"EventBus"),
        "missing EventBus TypeRef: {type_refs:?}"
    );
    assert!(
        type_refs.contains(&"Handler"),
        "missing Handler TypeRef: {type_refs:?}"
    );
}
...
"#;
    ...
    let type_refs: Vec<&str> = r
        ...
        .collect();
    assert!(
        type_refs.contains(&"Result"),
        "missing Result TypeRef: {type_refs:?}"
    );
}
...
"#;
    ...
    assert!(
        alias.is_some(),
        "missing SocketChannelPtr TypeAlias symbol: {:?}",
        r.symbols
    );
    ...
    let alias_idx = r
        .symbols
        .iter()
        .position(|s| s.name == "SocketChannelPtr")
        .unwrap();
    ...
    assert!(
        has_typeref,
        "SocketChannelPtr missing TypeRef to SocketChannel: {:?}",
        r.refs
    );
}
...
"#;
    ...
    let foo = foo.unwrap();
    assert!(
        foo.qualified_name.contains("hv"),
        "foo not scoped to hv, got: {}",
        foo.qualified_name
    );
}
...
"#;
    ...
    assert!(
        channel
            .unwrap()
            .qualified_name
            .contains("TcpClientEventLoopTmpl"),
        "channel not scoped to class, got: {}",
...
fn cpp_tcpclient_actual_file_parse_debug() {
    ...
    for sym in &r.symbols {
        if matches!(
            sym.name.as_str(),
            "hv" | "channel" | "TcpClientEventLoopTmpl" | "TSocketChannelPtr" | "TcpClient"
        ) {
            println!(
                "sym: name={}, qname={}, kind={:?}, line={}",
    ...
    let channel = r
        .symbols
        .iter()
        .find(|s| s.name == "channel" && s.qualified_name.contains("TcpClient"));
    assert!(
        channel.is_some(),
...
"#;
    ...
    assert_eq!(
        one_count, 1,
        "#define ONE emitted {one_count} times (expected 1)"
    );
    assert_eq!(
        two_count, 1,
        "#define TWO emitted {two_count} times (expected 1)"
    );
}
...
"#;
    ...
    assert_eq!(
        count, 1,
        "salvage must not duplicate; got {count} symbols for foo"
    );
}
...
fn template_class_decl_salvage_skips_unrelated_lines() {
    ...
    assert_eq!(
        count, 0,
        "template-prefixed call must not be salvaged as a class"
    );
}
...
fn declaration_macro_salvage_skips_function_pointer_declarator() {
    ...
    assert!(
        !r.symbols
            .iter()
            .any(|s| s.name == "Callback" && matches!(s.kind, SymbolKind::Function)),
