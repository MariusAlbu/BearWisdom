...
"#;
    ...
    assert!(
        ...
        "expected Calls ref to 'deploy_app'; got: {calls:?}"
    );
    assert!(
        calls.contains(&"notify_slack"),
        "expected Calls ref to 'notify_slack'; got: {calls:?}"
    );
}
...
fn extracts_local_declaration_inside_function() {
    ...
    assert!(
        count.is_some(),
        "expected Variable 'COUNT' from declare -i; got: {:?}",
        r.symbols
    );
}
...
fn extracts_command_substitution_inside_command_arg() {
    ...
    assert!(
        calls.contains(&"date"),
        "expected Calls ref to 'date' from embedded $(...); got: {calls:?}"
    );
}
...
fn source_variable_path_produces_import_ref_with_module() {
    ...
    assert!(
        !imports.is_empty(),
        "expected import ref for $VAR-path source"
    );
...
"#;
    ...
    assert!(
        names.contains(&"run_backup"),
        "expected run_backup call: {names:?}"
    );
    assert!(
