...
fn store_subscription_strips_dollar() {
    ...
    assert_eq!(svelte_store_base("$optionClickCallbackStore"), Some("optionClickCallbackStore"));
    assert_eq!(svelte_store_base("$_underscore"), Some("_underscore"));
}

#[test]
fn runes_are_not_stores() {
    for rune in ["$state", "$derived", "$effect", "$props", "$bindable", "$inspect", "$host"] {
        assert_eq!(svelte_store_base(rune), None, "{rune} is a rune, not a store");
    }
}
...
fn non_store_shapes_return_none() {
    ...
    assert_eq!(svelte_store_base("$1abc"), None, "must start with letter/underscore");
    assert_eq!(svelte_store_base("$page.url"), None, "dotted is not a bare identifier");
}
