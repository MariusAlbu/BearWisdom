...
"#;
    ...
    let names: Vec<&str> = r.symbols.iter().map(|s| s.name.as_str()).collect();
    assert!(
        r.symbols.iter().any(|s| s.name == "context"),
        "missing 'context': {:?}",
        names
    );
    assert!(
        r.symbols.iter().any(|s| s.name == "selectedTab"),
        "missing 'selectedTab': {:?}",
        names
    );
    assert!(
        r.symbols.iter().any(|s| s.name == "iosTabs"),
        "missing 'iosTabs': {:?}",
        names
    );
}
...
"#;
    ...
    let names: Vec<&str> = r.symbols.iter().map(|s| s.name.as_str()).collect();
    assert!(
        r.symbols.iter().any(|s| s.name == "b"),
        "missing 'b' from extension: {:?}",
        names
    );
    assert!(
        r.symbols.iter().any(|s| s.name == "label"),
        "missing 'label' from enum: {:?}",
        names
    );
    assert!(
        r.symbols.iter().any(|s| s.name == "bar"),
        "missing 'bar' function from extension: {:?}",
        names
    );
    assert!(
        r.symbols.iter().any(|s| s.name == "method"),
        "missing 'method' function from enum: {:?}",
        names
    );
}
...
"#;
    ...
    for (pname, tname) in [("g", "Greet"), ("e", "Greet"), ("x", "Plain")] {
        ...
        let p_idx = r.symbols.iter().position(|s| std::ptr::eq(s, p)).unwrap();
        // The parameter's FIRST TypeRef is its declared type (keyword peeled).
        let first_type = r
            .refs
            .iter()
            .find(|rf| rf.source_symbol_index == p_idx && rf.kind == EdgeKind::TypeRef)
