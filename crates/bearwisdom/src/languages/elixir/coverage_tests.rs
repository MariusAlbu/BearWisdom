...
fn ref_binary_operator_pipe() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "upcase" && rf.kind == EdgeKind::Calls),
...
fn ref_alias_node_type_ref() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "GenServer" && rf.kind == EdgeKind::TypeRef),
...
fn ref_alias_node_in_function_body() {
    ...
    assert!(
        r.refs.iter().any(|rf| rf.kind == EdgeKind::TypeRef
            && (rf.target_name == "Repo" || rf.target_name == "User")),
...
fn ref_pipe_operator_inside_function() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "filter" && rf.kind == EdgeKind::Calls),
...
fn ref_alias_multi_module() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "User" && rf.kind == EdgeKind::Imports),
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "Post" && rf.kind == EdgeKind::Imports),
...
fn symbol_at_type_attribute() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "@type" && s.kind == SymbolKind::Variable),
...
fn symbol_at_callback_attribute() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "@callback" && s.kind == SymbolKind::Variable),
...
fn ref_defimpl_typeref_to_protocol() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.target_name == "Enumerable" && rf.kind == EdgeKind::TypeRef),
...
fn ref_at_type_body_typeref() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.kind == EdgeKind::TypeRef && rf.target_name == "MyType"),
...
fn ref_at_spec_body_typeref() {
    ...
    assert!(
        r.refs.iter().any(|rf| rf.kind == EdgeKind::TypeRef
            && (rf.target_name == "Request" || rf.target_name == "Response")),
