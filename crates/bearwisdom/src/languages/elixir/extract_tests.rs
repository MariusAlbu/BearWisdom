...
"#;
    ...
    let module = r
        .symbols
        .iter()
        .find(|s| s.name == "MyApp.Greeter" || s.name == "Greeter")
        .expect("module");
...
"#;
    ...
    assert_eq!(
        ...
        "expected Interface kind for defprotocol"
    );
...
"#;
    ...
    assert!(
        ...
        "expected 'transform' call: {calls:?}"
    );
    assert!(
        calls.contains(&"validate"),
        "expected 'validate' call: {calls:?}"
    );
    assert!(
        calls.contains(&"persist"),
        "expected 'persist' call: {calls:?}"
    );
}
...
"#;
    ...
    assert!(
        imports
            .iter()
            .any(|(t, m)| *t == "H" && *m == Some("PlausibleWeb.Api.Helpers")),
...
"#;
    ...
    assert!(
        imports
            .iter()
            .any(|(t, m)| *t == "Foo" && *m == Some("MyApp.Foo")),
...
"#;
    let r = extract::extract(src);
    let calls: Vec<(&str, Option<&str>)> = r
        .refs
        .iter()
    ...
    assert!(
        !calls
            .iter()
            .any(|(t, m)| *t == "acquisition_channel" && *m == Some("session")),
        "lowercase 'session.acquisition_channel' must NOT emit Calls; got {calls:?}"
    );
    assert!(
        !calls
            .iter()
            .any(|(t, m)| *t == "html_body" && *m == Some("email")),
