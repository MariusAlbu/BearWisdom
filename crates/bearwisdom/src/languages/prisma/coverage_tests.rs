...
fn cov_model_declaration_produces_struct() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Struct && s.name == "User"),
...
fn cov_model_field_produces_field() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Field && s.name == "title"),
...
fn cov_enum_declaration_produces_enum() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Enum && s.name == "Role"),
...
fn cov_view_declaration_produces_class() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Class && s.name == "ActiveUsers"),
...
fn cov_generator_declaration_produces_variable() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Variable && s.name == "client"),
...
fn cov_type_declaration_produces_type_alias() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::TypeAlias && s.name == "Address"),
...
fn cov_scalar_field_types_no_type_ref() {
    ...
    let scalar_refs: Vec<&str> = r
        ...
        .collect();
    assert!(
        scalar_refs.is_empty(),
        "should not emit TypeRef for Prisma scalars; got: {scalar_refs:?}"
    );
}
...
fn cov_optional_field_emits_field() {
    ...
    let field = r
        .symbols
        .iter()
        .find(|s| s.name == "bio" && s.kind == SymbolKind::Field);
    assert!(
        field.is_some(),
        "expected Field 'bio' for optional field; got: {:?}",
        r.symbols
    );
    // String? → base type String is a scalar, so no TypeRef
    let has_string_ref = r
        .refs
        .iter()
        .any(|rf| rf.kind == EdgeKind::TypeRef && rf.target_name == "String");
    assert!(
        !has_string_ref,
        "should not emit TypeRef for optional scalar 'String?'; got: {:?}",
        r.refs
    );
}
...
fn cov_array_field_non_scalar_emits_type_ref() {
    ...
    let field = r
        .symbols
        .iter()
        .find(|s| s.name == "tags" && s.kind == SymbolKind::Field);
    assert!(
        field.is_some(),
        "expected Field 'tags' for array field; got: {:?}",
        r.symbols
    );
    let has_ref = r
        .refs
        .iter()
        .any(|rf| rf.kind == EdgeKind::TypeRef && rf.target_name == "Tag");
    assert!(
        has_ref,
        "expected TypeRef to 'Tag' from array field; got: {:?}",
        r.refs
    );
}
...
fn cov_doc_comment_captured_on_model() {
    ...
    let sym = r
        .symbols
        .iter()
        .find(|s| s.name == "Account" && s.kind == SymbolKind::Struct);
    assert!(
        sym.is_some(),
        "expected Struct 'Account'; got: {:?}",
        r.symbols
    );
...
fn cov_multiple_models_all_emitted() {
    ...
    for expected in &["User", "Post", "Comment"] {
        assert!(
            structs.contains(expected),
            "expected Struct '{expected}'; got: {structs:?}"
        );
    }
}
