...
/// walker — `<prefix>/rustlib/src/rust/library/<crate>/...`. The same subtree
/// the ambient-path marker keys the prelude on (`ecosystem/ambient.rs`), so a
/// bare prelude name (`Vec`, `Some`, `Default`) binds through the generic
/// ambient-package strategy.
