...
/// Rust `ChainConfig` for the unified `resolve_via_chain`.
///
/// Rust shares C's one delta: `normalize_type` rewrites `::` qualifiers to the
/// `.` separator the index keys on. `has_self_ref` is on so `Self::method()`
/// chains root on the enclosing struct/enum/trait via the scope chain.
/// `walk_inheritance` is on so trait-default / `extends`-reachable members
/// resolve on the final segment. No generics in the chain walk, no namespace
/// lookups, no alias expansion, no external-qname promotion, no construction
/// roots, no extension methods.
pub(crate) static RUST_CHAIN_CONFIG: crate::type_checker::chain::ChainConfig =
    crate::type_checker::chain::ChainConfig {
        strategy_prefix: "rust",
        normalize_type: predicates::normalize_path,
        has_self_ref: true,
        enclosing_type_kinds: &["struct", "enum", "trait", "class"],
        static_type_kinds: &["struct", "enum", "trait", "type_alias", "class"],
        use_generics: false,
...
pub(crate) fn resolve(
        ...
        if let Some(chain_val) = &ref_ctx.extracted_ref.chain {
            if let Some(res) = crate::type_checker::chain::resolve_via_chain(
                &RUST_CHAIN_CONFIG, chain_val, edge_kind, Some(file_ctx), ref_ctx, lookup,
            ) {
                return Some(res);
            }
        }
