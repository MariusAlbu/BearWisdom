...
pub(super) use crate::languages::common::{extract_call_args, replace_template_substitutions};
...
pub(super) fn extract_calls(
    ...
    for child in node.children(&mut cursor) {
        match child.kind() {
            ...
            // `x => x.foo` — an arrow with a member-access expression body emits
            // no Calls ref (the body isn't a call) and no other arm reaches the
            // bare `x.foo`. Emit a chain TypeRef for the body so the resolver
            // can type the leaf member against the callback parameter's seeded
            // type (`arr.map(x => x.foo)` binds `x` to the element type, then
            // resolves `foo` on it). A statement-block body (`x => { ... }`)
            // falls through to recursion, where its calls/members are handled.
            "arrow_function" => {
                if let Some(body) = child.child_by_field_name("body") {
                    emit_arrow_body_member_ref(&body, src, source_symbol_index, refs);
                }
                extract_calls(&child, src, source_symbol_index, refs);
            }
            _ => {
...
/// Emit a chain `TypeRef` for an arrow function's expression body when that
/// body is a `member_expression` (`x => x.foo`). `target_name` is the leaf
/// segment name and `chain` carries the full receiver path so the chain
/// walker resolves the member against the receiver's inferred type. Bodies
/// that aren't a member access (identifiers, literals, calls, statement
/// blocks) emit nothing — calls are covered by the `call_expression` arm and
/// the rest carry no member to resolve.
fn emit_arrow_body_member_ref(
    body: &Node,
    src: &[u8],
    source_symbol_index: usize,
    refs: &mut Vec<ExtractedRef>,
    ...
    let Some(chain) = build_chain(*body, src) else {
        return;
    };
    let target = chain
        .segments
    ...
    refs.push(ExtractedRef { is_import_binding: false, is_reexport: false,
        ...
        module: None,
        chain: Some(chain),
        byte_offset: body.start_byte() as u32,
        namespace_segments: Vec::new(),
        call_args: Vec::new(),
    });
}
