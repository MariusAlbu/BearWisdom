...
pub(super) fn _nextjs_routes_inner(conn: &Connection, project_root: &Path) -> Result<u32> {
    // Matches [param] and [...param] dynamic segments.
    let re_dynamic =
        Regex::new(r"\[\.\.\.(\w+)\]|\[(\w+)\]").expect("nextjs dynamic segment regex");
    // Matches exported HTTP method handlers in App Router route files.
    let re_method =
        Regex::new(r"export\s+(?:async\s+)?function\s+(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)\b")
