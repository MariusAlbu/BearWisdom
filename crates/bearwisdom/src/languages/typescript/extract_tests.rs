...
fn sym(source: &str) -> Vec<ExtractedSymbol> {
    extract::extract(source, false).symbols
}
fn refs(source: &str) -> Vec<ExtractedRef> {
    extract::extract(source, false).refs
}
...
"#;
    ...
    for ref_ in &r.refs {
        eprintln!(
            ...
            ref_.chain
                .as_ref()
                .and_then(|c| c.segments.first().map(|s| s.name.clone())),
...
fn extracts_import() {
    ...
    let r = refs(src);
    assert!(
        r.iter().any(|r| r.target_name == "CatalogService"),
        "refs: {r:?}"
    );
    let imp = r
        .iter()
        .find(|r| r.target_name == "CatalogService")
        .unwrap();
...
fn extracts_call() {
    ...
    assert!(
        calls.iter().any(|r| r.target_name == "fetchData"),
        "calls: {calls:?}"
    );
}
...
"#;
    ...
    let call = r
        .iter()
        .find(|r| r.kind == EdgeKind::Calls && r.target_name == "findOne");
...
"#;
    ...
    let call = r
        .iter()
        .find(|r| r.kind == EdgeKind::Calls && r.target_name == "fetchData");
    assert!(call.is_some(), "no Calls ref for fetchData; refs: {r:?}");
    assert_eq!(
        call.unwrap().module,
        None,
        "bare call should not get a module"
    );
}
...
"#;
    ...
    let call = r
        .iter()
        .find(|r| r.kind == EdgeKind::Calls && r.target_name == "doWork");
...
fn as_expression_generic_emits_base_type_ref() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Repository" && r.kind == EdgeKind::TypeRef),
        "refs: {r:?}"
    );
}
...
fn catch_clause_typed_emits_variable_and_type_ref() {
    ...
    assert!(
        s.iter()
            .any(|s| s.name == "e" && s.kind == SymbolKind::Variable),
        "symbols: {s:?}"
    );
    let r = refs(src);
    assert!(
        r.iter()
            .any(|r| r.target_name == "Error" && r.kind == EdgeKind::TypeRef),
        "refs: {r:?}"
    );
}
...
fn instanceof_in_method_emits_type_ref() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Handler" && r.kind == EdgeKind::TypeRef),
        "refs: {r:?}"
    );
}
...
fn catch_clause_untyped_emits_variable_only() {
    ...
    let r = refs(src);
    let type_refs_to_e: Vec<_> = r
        .iter()
        .filter(|r| r.kind == EdgeKind::TypeRef && r.target_name == "e")
        .collect();
    assert!(
        type_refs_to_e.is_empty(),
        "unexpected TypeRef for untyped catch: {type_refs_to_e:?}"
    );
}
...
fn namespace_emits_module_symbol() {
    ...
    assert!(
        s.iter()
            .any(|s| s.name == "MyNS" && s.kind == SymbolKind::Module),
        "symbols: {s:?}"
    );
}
...
fn namespace_members_are_nested() {
    ...
    assert!(
        helper.parent_index.is_some(),
        "helper should have a parent_index"
    );
    // The helper's qualified name should contain the helper name.
    assert!(
        helper.qualified_name.contains("helper"),
        "qname: {}",
...
fn generator_method_emits_method_symbol() {
    ...
    assert!(
        s.iter()
            .any(|s| s.name == "gen" && s.kind == SymbolKind::Method),
        "symbols: {s:?}"
    );
}
...
fn tagged_template_emits_call_ref() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "sql" && r.kind == EdgeKind::Calls),
        "refs: {r:?}"
    );
}
...
fn tagged_template_member_chain_emits_call() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "gql" && r.kind == EdgeKind::Calls),
        "refs: {r:?}"
    );
}
...
fn conditional_type_emits_type_refs() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Foo" && r.kind == EdgeKind::TypeRef),
...
fn typeof_type_emits_type_ref() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "MyClass" && r.kind == EdgeKind::TypeRef),
...
fn construct_signature_emits_method_symbol() {
    ...
    assert!(
        s.iter()
            .any(|s| s.kind == SymbolKind::Method || s.kind == SymbolKind::Constructor),
...
fn index_signature_emits_property_symbol() {
    ...
    assert!(
        s.iter()
            .any(|s| s.name.contains("key") && s.kind == SymbolKind::Property),
...
fn toplevel_call_emits_calls_ref() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "setupDatabase" && r.kind == EdgeKind::Calls),
...
fn field_initializer_call_emits_calls_ref() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "createLogger" && r.kind == EdgeKind::Calls),
...
fn new_expression_at_toplevel_emits_instantiates_ref() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "EventEmitter" && r.kind == EdgeKind::Instantiates),
...
fn new_expression_in_method_body_emits_instantiates_ref() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "Error" && r.kind == EdgeKind::Instantiates),
...
fn method_call_chain_toplevel_emits_calls_ref() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "log" && r.kind == EdgeKind::Calls),
...
fn dump_specific_node_kinds() {
    let snippets = [
        ...
        (
            "abstract_method",
            "abstract class A { abstract method(): void; }",
        ),
        (
            "construct_sig",
            "interface Factory { new(name: string): Product; }",
        ),
        ("call_sig", "interface Callable { (x: number): string; }"),
        ("keyof_type", "type Keys = keyof User;"),
        (
            "predicate_type",
            "function isAdmin(x: unknown): x is Admin { return true; }",
        ),
...
"#;
    ...
    let type_refs: Vec<&str> = all_refs
        .iter()
    ...
    assert!(
        ...
        "Should extract Repository from generic"
    );
    assert!(
        type_refs.contains(&"User"),
        "Should extract User from generic argument"
    );
    assert!(
        type_refs.contains(&"Map"),
        "Should extract Map from generic"
    );
    assert!(
        type_refs.contains(&"Item"),
        "Should extract Item from generic argument"
    );
}
...
fn inline_object_type_in_var_annotation() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "host" && s.kind == SymbolKind::Property),
...
fn inline_object_type_in_method_param() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "id" && s.kind == SymbolKind::Property),
...
fn reexport_named_emits_imports_ref() {
    ...
    assert!(
        r.is_some(),
        "named re-export should emit Imports ref with module set"
    );
}
...
fn reexport_named_alias_emits_original_name() {
    ...
    assert!(
        r.is_some(),
        "aliased re-export should emit Imports ref with original name"
    );
    // Alias ("Bar") should NOT appear as a target_name with module set.
    let alias_ref = result
        .refs
        .iter()
        .find(|r| r.kind == EdgeKind::Imports && r.target_name == "Bar");
    assert!(
        alias_ref.is_none(),
        "alias name should not appear as Imports ref target"
    );
}
...
fn reexport_star_emits_wildcard_ref() {
    ...
    assert!(
        r.is_some(),
        "export * should emit Imports ref with target_name='*'"
    );
}
...
fn reexport_star_as_emits_wildcard_ref() {
    ...
    assert!(
        r.is_some(),
        "export * as ns should emit Imports ref with target_name='*'"
    );
}
...
fn reexport_without_from_emits_no_imports_ref() {
    ...
    let r = result
        .refs
        .iter()
        .find(|r| r.kind == EdgeKind::Imports && r.target_name == "Foo");
    assert!(
        r.is_none(),
        "export without from should not emit Imports ref"
    );
}
...
fn reexport_multiple_named_emits_one_ref_per_export() {
    ...
    assert_eq!(
        import_refs.len(),
        3,
        "should emit one Imports ref per export specifier"
    );
}
...
fn bare_reexport_of_imported_name_emits_imports_ref() {
    ...
    let r = result.refs.iter().find(|r| {
        r.kind == EdgeKind::Imports && r.target_name == "Foo" && r.module.as_deref() == Some("pkg")
    });
    assert!(
        r.is_some(),
...
fn bare_reexport_with_alias_emits_canonical_target_and_alias_symbol() {
    ...
    let result = extract::extract("import { X } from 'pkg';\nexport { X as Y };", false);
    let r = result.refs.iter().find(|r| {
        r.kind == EdgeKind::Imports && r.target_name == "X" && r.module.as_deref() == Some("pkg")
    });
    assert!(
        r.is_some(),
        "alias re-export should preserve canonical target_name=X"
    );
    assert!(
...
fn bare_reexport_skips_local_declarations() {
    ...
    assert!(
        r.is_none(),
        "local re-export must not synthesize an Imports ref"
    );
}
...
fn explicit_type_annotation_takes_priority_over_new_expression() {
    ...
    assert!(
        has_typeref,
        "Expected at least one TypeRef 'UserService' from 'service'"
    );
}
...
fn jest_expect_to_equal_has_chain_ref() {
    ...
    let to_equal = r
        .iter()
        .find(|r| r.kind == EdgeKind::Calls && r.target_name == "toEqual");
    ...
    assert_eq!(
        chain.segments.len(),
        2,
        "chain should have 2 segments: [expect, toEqual]; got: {chain:?}"
    );
    assert_eq!(
        chain.segments[0].name, "expect",
        "chain root should be 'expect'"
    );
    assert_eq!(
        chain.segments[1].name, "toEqual",
        "chain last should be 'toEqual'"
    );
}
...
fn chai_bdd_expect_to_equal_has_chain_ref() {
    ...
    let equal = r
        .iter()
        .find(|r| r.kind == EdgeKind::Calls && r.target_name == "equal");
    ...
    assert_eq!(
        chain.segments.len(),
        3,
        "chain should have 3 segments: [expect, to, equal]; got: {chain:?}"
    );
    assert_eq!(
        chain.segments[0].name, "expect",
        "chain[0] should be 'expect'"
    );
    assert_eq!(chain.segments[1].name, "to", "chain[1] should be 'to'");
    assert_eq!(
        chain.segments[2].name, "equal",
        "chain[2] should be 'equal'"
    );
}
...
fn vitest_spy_mock_resolved_value_has_chain_ref() {
    ...
    let mrv = r
        .iter()
        .find(|r| r.kind == EdgeKind::Calls && r.target_name == "mockResolvedValue");
    assert!(
        mrv.is_some(),
        "no Calls ref for mockResolvedValue; refs: {r:?}"
    );
    ...
    assert_eq!(
        chain.segments[2].name, "mockResolvedValue",
        "chain[2] should be 'mockResolvedValue'"
    );
}
...
fn type_parameter_usage_inside_declaration_is_not_emitted_as_ref() {
    ...
    assert!(
        r.iter()
            .any(|r| r.target_name == "EventTarget" && r.kind == EdgeKind::TypeRef),
...
"#;
    let r = refs(src);
    let late: Vec<_> = r.iter().filter(|r| r.target_name == "late.d.ts").collect();
...
"#;
    let calls: Vec<String> = refs(src)
        ...
        .collect();
    assert!(
        calls.contains(&"foo".to_string()),
        "regression — clean call dropped: {calls:?}"
    );
}
...
fn exported_top_level_function_is_public() {
    ...
    let pubf = symbols
        ...
        .expect("publicFn extracted");
    let privf = symbols
        .iter()
        .find(|s| s.name == "privateFn")
        .expect("privateFn extracted");
    assert_eq!(
        pubf.visibility,
...
fn exported_class_is_public() {
    ...
    let pub_c = symbols
        ...
        .expect("PublicClass extracted");
    let priv_c = symbols
        .iter()
        .find(|s| s.name == "PrivateClass")
        .expect("PrivateClass extracted");
