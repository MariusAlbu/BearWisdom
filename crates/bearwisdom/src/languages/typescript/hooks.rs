// =============================================================================
// languages/typescript/hooks.rs — TypeScriptHooks impl plus the concrete
// TypeScriptResolver (workspace package lookup, tsconfig paths alias rewrite,
// module-relative + bare-import + DefinitelyTyped + deep-import qname
// stripping + barrel re-export follow, 	his. stripping + scope-chain +
// same-file + fully-qualified-name + field-type chain, npm-globals + TS-core-
// lib ambient-global fallback for bare single-identifier calls) + 30+ flow
// detectors (NestJS routes / gRPC / Injectable / Inject + Angular Injectable
// + decorator-driven config/feature-flag/db-query/mailer/mq/bgjob/RPC/tRPC +
// chain HTTP/db/RPC/route consumer + addService gRPC method expansion +
// member-access config/feature-flag synthetic Imports refs) + external
// classifier (DOM/React-namespace types, bare-specifier manifest match,
// alias-passthrough-via-barrel, ambient-global method index lookup) +
// build_file_context with NestJS controller-prefix pre-pass + bgjob queue
// binding refs all live in this file.
// =============================================================================
