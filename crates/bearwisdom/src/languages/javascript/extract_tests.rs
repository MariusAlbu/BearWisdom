...
fn import_statement_produces_type_ref() {
    ...
    assert!(
        targets.contains(&"useState"),
        "missing useState: {targets:?}"
    );
    assert!(
        targets.contains(&"useEffect"),
        "missing useEffect: {targets:?}"
    );
}
...
"#;
    ...
    assert!(
        ...
        "missing express import: {modules:?}"
    );
    assert!(
        modules.contains(&"path"),
        "missing path import: {modules:?}"
    );
}
...
"#;
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "on" && s.kind == SymbolKind::Method),
...
"#;
    ...
    assert!(
        ...
        "expected html tagged template call: {targets:?}"
    );
}
...
"#;
    ...
    eprintln!("Calls: {targets:?}");
    assert!(
        targets.contains(&"Server"),
        "expected new Server call: {targets:?}"
    );
}
...
"#;
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "err" && s.kind == SymbolKind::Variable),
...
"#;
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.name == "key" && s.kind == SymbolKind::Variable),
...
"#;
    ...
    assert!(
        ...
        "expected new Connection call: {targets:?}"
    );
}
...
"#;
    ...
    assert!(
        targets.contains(&"fetchData"),
        "expected fetchData call: {targets:?}"
    );
}
...
fn member_call_produces_chain_ref() {
    ...
    let chain = spy_ref
        .chain
        .as_ref()
        .expect("chain must be Some for member call");
    assert_eq!(
        chain.segments.len(),
        2,
        "expected [vi, spyOn], got {:?}",
        chain.segments
    );
...
fn chained_assertion_produces_chain_ref() {
    ...
    let chain = matcher_ref
        .chain
        .as_ref()
        .expect("chain must be Some for chained assertion");
    assert!(
        chain.segments.len() >= 2,
        "expected at least [expect, toHaveBeenCalledOnce], got {:?}",
        chain.segments
    );
...
fn long_chai_chain_produces_chain_ref() {
    ...
    let chain = equal_ref
        .chain
        .as_ref()
        .expect("chain must be Some for chai chain");
    assert!(
        chain.segments.len() >= 2,
        "expected multi-segment chain, got {:?}",
        chain.segments
    );
...
"#;
    let r = extract::extract(src);
    assert!(
        r.symbols.iter().any(|s| s.name == "dayjs"
            && s.kind == SymbolKind::Function
            && s.parent_index.is_none()),
