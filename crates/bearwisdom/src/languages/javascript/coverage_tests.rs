...
fn coverage_export_statement() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Class && s.name == "Router"),
...
fn coverage_arrow_function_symbol() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Function && s.name == "double"),
...
fn coverage_method_call_member_expression() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|r| r.kind == EdgeKind::Calls && r.target_name.contains("log")),
...
fn coverage_object_destructuring_variable() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Variable && s.name == "name"),
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Variable && s.name == "age"),
...
fn coverage_array_destructuring_variable() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Variable && s.name == "head"),
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Variable && s.name == "tail"),
