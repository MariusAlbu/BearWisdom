...
fn cov_element_produces_component_class_symbol() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Class && s.name == "BlogPost"),
...
fn cov_element_pascal_produces_calls() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.kind == EdgeKind::Calls && rf.target_name == "Counter"),
...
fn cov_client_idle_directive_component_produces_calls() {
    ...
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.kind == EdgeKind::Calls && rf.target_name == "HeavyWidget"),
...
fn cov_fragment_builtin_does_not_produce_calls() {
    ...
    assert!(
        !r.refs
            .iter()
            .any(|rf| rf.kind == EdgeKind::Calls && rf.target_name == "Fragment"),
...
fn cov_slot_builtin_does_not_produce_calls() {
    ...
    assert!(
        !r.refs
            .iter()
            .any(|rf| rf.kind == EdgeKind::Calls && rf.target_name == "slot"),
...
fn cov_nested_component_inside_html_produces_calls() {
    ...
    let r = extract::extract("<main><div><BlogCard /></div></main>", "Index.astro");
    assert!(
        r.refs
            .iter()
            .any(|rf| rf.kind == EdgeKind::Calls && rf.target_name == "BlogCard"),
...
fn cov_component_symbol_simple_name() {
    ...
    assert!(
        r.symbols
            .iter()
            .any(|s| s.kind == SymbolKind::Class && s.name == "index"),
