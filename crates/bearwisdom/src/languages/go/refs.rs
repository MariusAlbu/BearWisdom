...
pub(super) fn extract_refs_from_body(
    ...
    for child in node.children(&mut cursor) {
        dispatch_ref_node(child, source, source_symbol_index, refs);
    }
}

/// Apply the per-node-kind ref extraction to a single node — the same
/// handling `extract_refs_from_body` applies to each child in its walk.
/// Lets a caller dispatch one specific subtree (e.g. a type assertion's
/// operand) without re-walking every sibling in its containing node.
fn dispatch_ref_node(
    child: Node,
    source: &str,
    source_symbol_index: usize,
    refs: &mut Vec<ExtractedRef>,
    ...
    match child.kind() {
        ...
        "type_assertion_expression" => {
            ...
            // Dispatch only the operand — the asserted type is already
            // emitted above and must not be walked again.
            if let Some(operand) = child.child_by_field_name("operand") {
                dispatch_ref_node(operand, source, source_symbol_index, refs);
            }
        }
        ...
        // Each case's type(s) are emitted by `extract_type_switch_refs` when
        // the enclosing `type_switch_statement` is dispatched above; only the
        // case body (wrapped in a `statement_list`) needs a walk here.
        "type_case" | "default_case" => {
            let mut ccursor = child.walk();
            for case_child in child.children(&mut ccursor) {
                if case_child.kind() == "statement_list" {
                    extract_refs_from_body(&case_child, source, source_symbol_index, refs);
                    break;
                }
            }
        }
        ...
        // `pkg.Type` in a type position — emit a TypeRef for the bare member
        ...
        "func_literal" => {
            ...
            // Recurse only into the body block — parameter and result types
            // are already covered above; walking the whole node here would
            // re-visit them.
...
pub(super) fn extract_func_literal_type_refs(
    node: &Node,
    source: &str,
    source_symbol_index: usize,
    refs: &mut Vec<ExtractedRef>,
