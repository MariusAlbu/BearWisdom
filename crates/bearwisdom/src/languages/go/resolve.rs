...
// Re-export the detectors at crate visibility so this module (the
// LanguageResolver impl below) and resolve_tests.rs (which reaches into
// them via `super::resolve::detect_go_*`) both keep working after the
// carve. `pub(crate) use` doubles as the module's own `use` import.
