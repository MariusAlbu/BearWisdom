...
/// Go gin/chi/net-http route scan + `routes` table population. The
...
pub fn discover_go_routes(conn: &Connection, project_root: &Path, ctx: &ProjectContext) -> u32 {
    ...
    let routes = match extract_go_routes_pub(conn, project_root) {
        Ok(r) => r,
        Err(e) => {
            tracing::warn!("Go route detection failed: {e}");
            return 0;
        }
    };

    match write_routes(conn, &routes) {
        Ok(n) => n,
        Err(e) => {
            tracing::warn!("Go route insert failed: {e}");
            0
        }
    }
}
