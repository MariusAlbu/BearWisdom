...
pub struct LanguageProfile {
    ...
    /// Symbol kinds a bare, unqualified target may bind to in the ROOT
    /// namespace after the file's own namespace missed. Empty (the default)
    /// leaves the rung inert. A kind SET, not a flag: a language whose
    /// functions and constants fall back to the root while its types do not is
    /// expressible as data. An edge-kind set could not express it — a constant
    /// rides whatever edge kind its reference carries — and the
    /// `kind_compatible_table` still gates edge→kind on top of this set.
    pub root_namespace_fallback: &'static [SymbolKind],
