...
// Interning dedups in the destination, so merging is idempotent and two
// sources sharing structurally equal types converge on one TypeId.
// =============================================================================
...
pub fn merge_snapshot_into(
    ...
    for i in 0..n {
        let src_id = TypeId(std::num::NonZeroU32::new((i + 1) as u32).expect("arena index overflow"));
        let remapped = remap_type(src.get(src_id), &types, &mut generics, &src, dst, remap_decl_id);
...
fn remap_type(
    ...
    match ty {
        ...
        Type::Decl { symbol_id, qname } => Type::Decl {
        ...
        Type::Generic { param } => Type::Generic {
            param: *generics.entry(param).or_insert_with(|| {
