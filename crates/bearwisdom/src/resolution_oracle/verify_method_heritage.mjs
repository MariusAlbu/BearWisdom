...
const cases = JSON.parse(readFileSync(new URL('method_heritage_fixtures.json', import.meta.url), 'utf8'));
let exactCallTargets = 0;
for (const test of cases) {
  for (const prefix of ['', 'export {}; ']) {
    const path = resolve('bearwisdom-methods-virtual.ts').replaceAll('\\', '/');
