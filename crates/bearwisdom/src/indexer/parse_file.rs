...
fn parse_file_internal(
    ...
    // zero cost here.
    // Project source gets a symbol for every binding the flow queries name;
    // external supply keeps only what its extractor emitted (bodies are
    // dropped by the contract filter downstream).
    let bindings = if super::ext_virtual_path::is_virtual_external(&walked.relative_path) {
        crate::indexer::flow::BindingSymbols::CorrelateOnly
    } else {
    ...
    // The object type a literal-returning function yields, materialized as a
    // `{fn}$Ret` interface with one property per returned name.
    super::return_object_types::materialize(
        &mut r.symbols,
        std::mem::take(&mut flow_meta.flow_return_object),
    );
    // Synthesized symbols (bindings, return-object types) extend the symbol
    // list; keep the parallel origin vectors aligned when they were populated
    ...
    let plugin_flow_emissions = plugin.plugin_flow_emissions(&content, &walked.relative_path);
