...
/// Ruby built-in kernel methods always in scope.
fn is_ruby_builtin(name: &str) -> bool {
    let root = name.split('.').next().unwrap_or(name);
    matches!(
        root,
        "puts" | "print" | "raise" | "require" | "require_relative"
            | "include" | "extend" | "attr_reader" | "attr_writer" | "attr_accessor"
            | "define_method" | "lambda" | "proc" | "block_given?"
            | "p" | "pp" | "sleep" | "exit" | "abort"
            | "Array" | "Integer" | "Float" | "String" | "Hash"
