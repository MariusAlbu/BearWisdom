use super::{
    ...
    parse_param_types_from_signature_for_lang, parse_return_type_from_jvm_descriptor,
    parse_return_type_from_signature, parse_return_type_from_signature_for_lang,
    parse_return_type_positional, parse_return_type_trailing, parse_type_head_and_args,
    parse_type_head_and_args_bracket,
};
...
fn declaration_variance_keyword_is_not_the_name() {
    ...
    assert_eq!(
        parse_generic_param_clause("out T"),
        vec![("T".to_string(), None)]
    );
    assert_eq!(
        parse_generic_param_clause("in T"),
        vec![("T".to_string(), None)]
    );
}
...
fn where_clause_fills_unbounded_param() {
    ...
    assert_eq!(
        params,
        vec![("T".to_string(), Some("IComparable".to_string()))]
    );
}
...
fn rust_single_line_where_clause() {
    ...
    merge_where_bounds(
        &mut params,
        "fn f<T, U>(x: T, y: U) where T: Clone + Send, U: Debug",
    );
    assert_eq!(
        params,
        vec![
...
fn bracket_single_and_multi_arg() {
    assert_eq!(
        parse_type_head_and_args_bracket("List[User]"),
        ("List", vec!["User"])
    );
    assert_eq!(
...
fn bracket_leading_is_not_an_application() {
    ...
    assert_eq!(
        parse_type_head_and_args_bracket("[]User"),
        ("[]User", Vec::new())
    );
    assert_eq!(
        parse_type_head_and_args_bracket("[A, B]"),
        ("[A, B]", Vec::new())
    );
}
...
fn bracket_plain_name() {
    assert_eq!(
        parse_type_head_and_args_bracket("Result"),
        ("Result", Vec::new())
    );
}
...
fn bracket_rejects_trailing_text() {
    ...
    assert_eq!(
        parse_type_head_and_args_bracket("map[K]V"),
        ("map[K]V", Vec::new())
    );
}
...
fn trailing_go_func_and_method() {
    assert_eq!(
        parse_return_type_trailing("func F(a A) Ret"),
        Some("Ret".to_string())
    );
    assert_eq!(
    ...
    assert_eq!(
        parse_return_type_trailing("func (s *S) Get() []User"),
        Some("[]User".to_string())
    );
}
