...
//
// Try `{scope}.{target}` for each canonical scope in the ref's scope chain
// (innermost first). Catches a local symbol defined in the source
// symbol's own enclosing type / namespace / module before resolution widens to
// file or project scope.
//
// Source-specific separators have already been converted at the language
// profile boundary. `name_normalization` only fires its fallback for a folding
// language; a case-sensitive language is the exact probe alone.
// =============================================================================
...
impl LookupRule for ScopeVisibleRule {
    ...
    fn apply(&self, ctx: &BinderContext) -> LookupResult {
        ...
        for scope in &ctx.ref_ctx.scope_chain {
            let qname = crate::indexer::resolve::engine::support::join_index_qname(scope, target);
            if let Some(sym) = ctx.lookup.by_qualified_name(&qname) {
                if (ctx.kind)(edge_kind, &sym.kind) {
                    return LookupResult::Resolved(ctx.resolved(sym.id, "default_scope_visible"));
                }
