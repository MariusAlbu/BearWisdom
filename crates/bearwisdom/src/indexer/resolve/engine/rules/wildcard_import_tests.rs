...
#[test]
fn package_root_mode_reads_package_identity_and_stem_from_the_uri_spelling() {
    // The wildcard keeps the import's own spelling; the Dart adapter reads the
    // package identity for an external candidate and the library stem for an
    // internal one out of the URI.
    let lookup = Lookup::new()
        .with(sym(
            10,
            "BuildContext",
        ...
        .with(sym(
            ...
            "Bar",
            "class",
            "packages/app/lib/widgets.dart",
        ));
    let external = vec![wildcard_import("*", "package:flutter/material.dart")];
    assert_eq!(
        resolve_with_profile(
            &lookup,
            "BuildContext",
    ...
    let relative = vec![wildcard_import("*", "../widgets.dart")];
    assert_eq!(
        resolve_with_profile(
            &lookup,
            "Bar",
