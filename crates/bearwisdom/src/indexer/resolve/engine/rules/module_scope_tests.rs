...
fn passes_when_gate_off() {
    ...
    assert_eq!(
        resolve(&lookup, "Vec2", "pkg/math/main.odin", &DEFAULT_PROFILE),
        None
    );
}
...
fn same_dir_resolves_sibling() {
    ...
    assert_eq!(
        resolve(&lookup, "Vec2", "pkg/math/main.odin", &PROFILE),
        Some(10)
    );
}
...
fn same_dir_unique_resolves_single_candidate() {
    ...
    assert_eq!(
        resolve(&lookup, "init", "src/app/b.pas", &PROFILE),
        Some(30)
    );
}
...
#[test]
fn sources_target_subtree_uses_profile_roots() {
    static PROFILE: crate::type_checker::profile::language_profile::LanguageProfile =
        crate::type_checker::profile::language_profile::LanguageProfile {
            implicit_root_types: &[],
            imports: crate::type_checker::profile::language_profile::ImportAxes {
