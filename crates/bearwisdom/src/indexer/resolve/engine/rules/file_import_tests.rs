...
fn binds_directly_imported_name() {
    let lookup = Lookup::new().with(sym(10, "Foo", "Foo", "class", "src/foo.ts"));
...
fn binds_via_alias_using_original_name() {
    ...
    let lookup = Lookup::new().with(sym(11, "Foo", "Foo", "class", "src/foo.ts"));
...
fn declines_when_no_import_matches_target() {
    let lookup = Lookup::new().with(sym(12, "Baz", "Baz", "class", "src/baz.ts"));
...
fn declines_when_file_path_does_not_match_module() {
    let lookup = Lookup::new().with(sym(13, "Foo", "Foo", "class", "src/unrelated.ts"));
    let imports = vec![import("Foo", Some("./foo"))];
