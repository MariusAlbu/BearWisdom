...
fn bicep_bare_arm_builtin_resolves_over_same_name_internal_declaration() {
    ...
    assert_eq!(
        resolve_for(&lookup, "resourceGroup", &BICEP_PROFILE),
        Some(20)
    );
}
...
fn bicep_bare_decorator_resolves_through_decorators_namespace() {
    ...
    let lookup = Lookup::new().with(sym(
        22,
        "description",
        "bicep.decorators.description",
        "function",
        "ext:bicep-runtime:namespace.bicep",
    ));
    assert_eq!(
        resolve_for(&lookup, "description", &BICEP_PROFILE),
        Some(22)
    );
}
