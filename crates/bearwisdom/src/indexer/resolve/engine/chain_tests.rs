...
#[test]
fn a_mapped_type_over_a_literal_union_admits_those_keys() {
    // `type Keys = 'click' | 'change'; type Obj = { [K in Keys]: Handler }`
    // — `.click` is generated by the mapping, declared nowhere, so the walk
    // has to read the source union's literals to admit it.
    let lookup = Lookup::new()
        .with_local_type("fire", "Obj")
        .with(sym(1, "Obj", "Obj", "type_alias", "ext:ts:dom.d.ts"))
        .with_alias(
            "Obj",
            AliasTarget::Mapped {
                source: "Keys".to_string(),
                value_template: "Handler".to_string(),
            },
        )
        .with(sym(2, "Keys", "Keys", "type_alias", "ext:ts:dom.d.ts"))
        .with_alias(
            "Keys",
            AliasTarget::Union(vec!["'click'".to_string(), "'change'".to_string()]),
        );
    let hit = vec![
        seg("fire", false, SegmentKind::Identifier),
        seg("click", true, SegmentKind::Property),
    ];
    let miss = vec![
        seg("fire", false, SegmentKind::Identifier),
        seg("submit", true, SegmentKind::Property),
    ];
