...
fn uri_scheme_strips() {
    assert_eq!(strip_scheme_prefix("custom:assert/strict"), Some("assert/strict"));
    assert_eq!(strip_scheme_prefix("virtual:module"), Some("module"));
...
fn non_scheme_shapes_pass_through() {
    ...
    assert_eq!(strip_scheme_prefix("custom:"), None);
