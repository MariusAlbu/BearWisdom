...
fn a_value_carrying_the_head_qname_is_not_the_receiver_declaration() {
    ...
    assert_eq!(
        head_symbol_id_preferring_package(&arena, &lookup, ty, None),
        None
    );
}
...
fn a_type_wins_over_a_value_sharing_one_qname() {
    ...
    let arena = TypeArena::new();
    assert_eq!(
        head_symbol_id(&arena, &lookup, arena.class("D"), None),
        Some(2)
    );
}
...
fn a_head_naming_nothing_stays_unbound() {
    ...
    let arena = TypeArena::new();
    assert_eq!(
        head_symbol_id(&arena, &lookup, arena.class("Missing"), None),
        None
    );
}
...
fn head_decl_id_carries_through_wrappers_and_apply() {
    ...
    assert_eq!(
        super::head_decl_id(&arena, arena.class("Repo")),
        None,
        "name-only heads carry nothing"
    );
