...
// materialized symbol also matches does it collect `&Symbol` across stores.
// =============================================================================
...
impl<'a> SymbolSet<'a> {
    ...
    pub fn last(&self) -> Option<&'a Symbol> {
        match self {
    ...
    pub fn get(&self, i: usize) -> Option<&'a Symbol> {
        match self {
    ...
    /// Clone the symbols out into an owned `Vec<Symbol>`.
...
impl<'a> From<&'a [Symbol]> for SymbolSet<'a> {
...
impl<'a> From<Vec<&'a Symbol>> for SymbolSet<'a> {
    fn from(v: Vec<&'a Symbol>) -> Self {
        SymbolSet::Owned(v)
    }
}
...
pub enum SymbolSetIter<'a> {
    Slice(std::slice::Iter<'a, Symbol>),
    Refs(std::slice::Iter<'a, &'a Symbol>),
}
