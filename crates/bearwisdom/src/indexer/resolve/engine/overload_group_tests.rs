...
use super::super::member_selection::{
    select, select_representing, select_typed, select_typed_representing, Selection,
};
...
fn one_owners_callable_rows_collapse_to_their_lowest_id() {
    ...
    assert_eq!(
        select_representing(&lookup, 1, name, &|_| true),
        Selection::Unique(71)
    );
}
...
fn the_kind_predicate_narrows_the_group_before_it_collapses() {
    ...
    assert_eq!(
        select_representing(&lookup, 1, name, &calls),
        Selection::Unique(72)
    );
    assert_eq!(
        select_representing(&lookup, 1, name, &|_| true),
...
fn the_typed_entry_collapses_an_overload_set_for_the_chain_walk() {
    ...
    assert_eq!(
        select_typed_representing(&lookup, &arena, recv, name, &|_| true),
        Selection::Unique(80)
    );
    let walked = super::super::implicit_root::walk_member(
        ...
        recv,
        "add",
        &crate::languages::java::JAVA_PROFILE,
        &|_| true,
    )
    .unwrap()
    .unwrap();
