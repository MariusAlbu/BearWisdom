...
#[test]
fn engine_selects_an_rbi_contract_before_typing_a_ruby_trailing_block() {
    let dir = tempfile::tempdir().unwrap();
    let rbi_path = dir.path().join("catalog.rbi");
    let ruby_path = dir.path().join("callbacks.rb");
    std::fs::write(
        &rbi_path,
        "class Catalog\n  sig { params(block: T.proc.params(item: Item).returns(Result)).returns(Response) }\n  def visit(&block); end\nend\n",
    )
    .unwrap();
    std::fs::write(
        &ruby_path,
    ...
    assert!(
        edges.iter().any(|edge| edge.1 == visit_id),
        "the Ruby call must select the indexed RBI contract; edges={edges:?}; unresolved={unresolved:?}"
    );
    assert!(
        edges.iter().any(|edge| edge.1 == touch_id),
