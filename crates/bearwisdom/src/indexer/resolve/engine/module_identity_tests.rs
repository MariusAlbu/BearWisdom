...
fn project_granularity_shares_one_container() {
    ...
    assert_eq!(
        module_discriminator(ModuleGranularity::Project, "a/b.ts"),
        ""
    );
    assert_eq!(
        module_discriminator(ModuleGranularity::Project, "c/d.ts"),
        ""
    );
}
