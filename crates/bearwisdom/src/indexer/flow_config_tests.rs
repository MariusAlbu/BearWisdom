...
fn check_plugin(lang_id: &str) {
    let plugin = default_registry()
        .get_dedicated(lang_id)
        .unwrap_or_else(|| panic!("plugin missing for {lang_id}"));
    let Some(cfg) = plugin.flow_config() else {
        return;
    };
    let grammar = plugin
        .grammar(lang_id)
    ...
    if !cfg.assignment_query.trim().is_empty() {
        Query::new(&grammar, cfg.assignment_query)
            .unwrap_or_else(|e| panic!("{lang_id} assignment_query failed to compile: {e:?}"));
    }
...
#[test]
fn rust_flow_queries_compile() {
    check_plugin("rust");
}
#[test]
fn java_flow_queries_compile() {
    check_plugin("java");
}
#[test]
fn kotlin_flow_queries_compile() {
    check_plugin("kotlin");
}
#[test]
fn scala_flow_queries_compile() {
    check_plugin("scala");
}
#[test]
fn csharp_flow_queries_compile() {
    check_plugin("csharp");
}
#[test]
fn go_flow_queries_compile() {
    check_plugin("go");
}
#[test]
fn php_flow_queries_compile() {
    check_plugin("php");
}
#[test]
fn ruby_flow_queries_compile() {
    check_plugin("ruby");
}
#[test]
fn c_flow_queries_compile() {
    check_plugin("c");
}
#[test]
fn lua_flow_queries_compile() {
    check_plugin("lua");
}
