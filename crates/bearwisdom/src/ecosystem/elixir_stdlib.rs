...
fn discover() -> Vec<ExternalDepRoot> {
    ...
    let Ok(entries) = std::fs::read_dir(&install_lib) else {
        return roots;
    };
...
fn probe_elixir_install_lib() -> Option<PathBuf> {
    ...
    let probe_via = |program: &str, prefix_args: &[&str]| -> Option<PathBuf> {
        ...
        if install_lib.is_dir() {
            Some(install_lib.to_path_buf())
        } else {
            None
        }
    };

    if let Some(p) = probe_via("elixir", &[]) {
        return Some(p);
    }
    #[cfg(windows)]
    {
        if let Some(p) = probe_via("elixir.bat", &[]) {
            return Some(p);
        }
        if let Some(p) = probe_via("cmd", &["/C", "elixir"]) {
            return Some(p);
        }
    }
    None
...
pub fn shared_locator() -> Arc<dyn ExternalSourceLocator> {
    ...
    LOCATOR
        .get_or_init(|| Arc::new(ElixirStdlibEcosystem))
        .clone()
}
