...
fn hit_does_not_reload() {
    ...
    assert_eq!(
        cache.get_or_load(Path::new("a.dll"), counting_load(&calls, Some((1, 10)))),
        Some(1)
    );
    assert_eq!(
        cache.get_or_load(Path::new("a.dll"), counting_load(&calls, Some((1, 10)))),
...
fn failed_load_is_not_retried_until_clear() {
    ...
    assert_eq!(
        cache.get_or_load(Path::new("bad.dll"), counting_load(&calls, Some((9, 1)))),
        None
    );
    assert_eq!(calls.get(), 1);
}
...
fn clear_forgets_successes_failures_and_budget_use() {
    ...
    cache.clear();
    assert_eq!(
        cache.get_or_load(Path::new("a.dll"), counting_load(&calls, Some((2, 8)))),
        Some(2)
    );
    assert_eq!(
        cache.get_or_load(Path::new("bad.dll"), counting_load(&calls, Some((3, 1)))),
        Some(3)
    );
