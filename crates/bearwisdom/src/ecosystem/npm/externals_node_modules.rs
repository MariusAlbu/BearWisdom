...
pub(crate) fn read_single_package_json_deps(
    dir: &Path,
...
pub(crate) fn walk_for_package_json(
    ...
    for entry in entries.flatten() {
        ...
        if file_type.is_dir() {
            ...
            if let Some(deps) = read_single_package_json_deps(entry.path().parent().unwrap_or(cur))
            {
                out.extend(deps);
            }
        }
...
pub(crate) fn find_node_modules_with_ancestors(
    start: &Path,
    workspace_root: &Path,
) -> Vec<PathBuf> {
...
pub(crate) fn find_node_modules(project_root: &Path) -> Vec<PathBuf> {
    ...
    // The walk yields node_modules pockets in filesystem/walk order, which is
    // not stable; the resulting dep-root order steers first-writer-wins. Sort so
    // the materialized external set is a deterministic function of the inputs.
    out.sort();
    out
}
