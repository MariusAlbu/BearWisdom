...
fn symbol_index_scans_pas_and_pp_but_not_non_pascal_files() {
    ...
    assert!(
        idx.locate("lcl", "forms").is_some(),
        "forms.pas must be scanned"
    );
    assert!(
        idx.locate("lcl", "buttons").is_some(),
        "buttons.pp must be scanned"
    );
    // Non-Pascal files must not produce entries.
    assert!(
        idx.locate("lcl", "readme").is_none(),
        "README.md must not be indexed"
    );
}
...
fn symbol_index_skips_tests_and_examples_dirs() {
    ...
    fs::write(
        root.join("tests").join("test_forms.pas"),
        "unit TestForms;\ninterface\nimplementation\n",
    )
    .unwrap();
    fs::write(
        root.join("examples").join("hello.pas"),
        "unit Hello;\ninterface\nimplementation\n",
    )
    .unwrap();
    fs::write(
        root.join("demos").join("demo.pas"),
        "unit Demo;\ninterface\nimplementation\n",
    )
    .unwrap();
    fs::write(
    ...
    assert!(
        idx.locate("lcl", "forms").is_some(),
        "top-level forms.pas must be indexed"
    );
    assert!(
        idx.locate("lcl", "testforms").is_none(),
        "tests/ dir must be skipped"
    );
    assert!(
        idx.locate("lcl", "hello").is_none(),
        "examples/ dir must be skipped"
    );
    assert!(
        idx.locate("lcl", "demo").is_none(),
        "demos/ dir must be skipped"
    );
}
...
";
    ...
    // Unit name.
    assert!(
        idx.locate("lcl", "myunit").is_some(),
        "unit name must be indexed"
    );
    // Interface declarations.
    assert!(
        idx.locate("lcl", "tmyclass").is_some(),
        "type must be indexed"
    );
    assert!(
        idx.locate("lcl", "dosomething").is_some(),
        "procedure must be indexed"
    );
    assert!(
        idx.locate("lcl", "getvalue").is_some(),
        "function must be indexed"
    );
    assert!(
        idx.locate("lcl", "max_items").is_some(),
        "const must be indexed"
    );
    assert!(
        idx.locate("lcl", "globalflag").is_some(),
        "var must be indexed"
    );
...
fn symbol_index_scans_inc_fragments_without_unit_wrapper() {
    ...
    assert!(
        idx.locate("fpc-rtl-inc", "tobject").is_some(),
        "TObject must resolve from the .inc fragment"
    );
    assert!(
        idx.locate("fpc-rtl-inc", "getmem").is_some(),
        "GetMem must resolve from the .inc fragment"
    );
}
...
fn extract_decl_ident_recognises_keywords() {
    assert_eq!(
        extract_decl_ident("procedure dosomething(x: integer)"),
        Some("dosomething")
    );
    assert_eq!(
        extract_decl_ident("function getvalue: string"),
        Some("getvalue")
    );
    assert_eq!(
        extract_decl_ident("type tmyclass = class"),
        Some("tmyclass")
    );
    assert_eq!(
        extract_decl_ident("var globalflag: boolean"),
        Some("globalflag")
    );
