    ...
    fn build_symbol_index(
        ...
        for dep in dep_roots {
            for wf in super::maven::walk_generic_jvm_root(dep) {
                ...
                let Ok(source) = std::fs::read_to_string(&wf.absolute_path) else {
                    continue;
                };
...
fn discover() -> Vec<ExternalDepRoot> {
    ...
    let Ok(versions) = std::fs::read_dir(&artifact_dir) else {
        return Vec::new();
    };
    let mut vs: Vec<PathBuf> = versions
        ...
        .collect();
    vs.sort();
    let Some(latest) = vs.into_iter().next_back() else {
        return Vec::new();
    };
    let Ok(files) = std::fs::read_dir(&latest) else {
        return Vec::new();
    };
    let jar = files
        .flatten()
    ...
    let cache_base = repo
        .parent()
        .unwrap_or(&repo)
        .join("bearwisdom-clojure-core-cache");
...
pub fn shared_locator() -> Arc<dyn ExternalSourceLocator> {
    ...
    LOCATOR
        .get_or_init(|| Arc::new(ClojureCoreEcosystem))
        .clone()
}
