...
pub fn shared_locator() -> Arc<dyn ExternalSourceLocator> {
    ...
    LOCATOR
        .get_or_init(|| Arc::new(BazelCentralRegistryEcosystem))
        .clone()
}
...
pub fn discover_bazel_externals(project_root: &Path) -> Vec<ExternalDepRoot> {
    ...
    if externals_dirs.is_empty() {
        debug!(
            "BazelBCR: no external/ directories found for {}",
    ...
    for ext_dir in &externals_dirs {
        ...
        for entry in entries.flatten() {
            ...
            let Some(dep_name) = path.file_name().and_then(|n| n.to_str()) else {
                continue;
            };
            // Skip Bazel-internal repos prefixed with `_`, `bazel_tools`, etc.
            if dep_name.starts_with('_')
                || dep_name == "bazel_tools"
                || dep_name == "local_config_cc"
            {
                continue;
            }
            ...
            if !declared.is_empty() && !declared.contains(dep_name) {
                continue;
            }
...
fn declared_dep_names(project_root: &Path) -> std::collections::HashSet<String> {
    ...
    if let Ok(content) = std::fs::read_to_string(&module_path) {
        for n in extract_bzlmod_deps(&content) {
            names.insert(n);
        }
    }
    ...
    for candidate in ["WORKSPACE", "WORKSPACE.bazel"] {
        ...
        if let Ok(content) = std::fs::read_to_string(&ws_path) {
            for n in extract_workspace_deps(&content) {
                names.insert(n);
            }
            break;
        }
...
fn find_output_base_externals() -> Vec<PathBuf> {
    ...
    let bazel_dir = if cfg!(windows) {
        cache_root.join(".bazel")
    } else {
        cache_root
    };
    if !bazel_dir.is_dir() {
    ...
    let Ok(user_dirs) = std::fs::read_dir(&bazel_dir) else {
        return found;
    };
    for user_entry in user_dirs.flatten() {
        ...
        let Ok(hash_dirs) = std::fs::read_dir(&user_path) else {
            continue;
        };
...
const REPOSITORY_CTX_MEMBERS: &[(&str, &str, &str)] = &[
    (
        ...
        "repository_ctx.execute(arguments, timeout=600, environment={}, **kwargs) -> exec_result",
    ),
    (
        "path",
        "repository_ctx.path",
        "repository_ctx.path(path) -> path",
    ),
    (
        "download",
        "repository_ctx.download",
        "repository_ctx.download(url, output, sha256='', **kwargs)",
    ),
    (
        "download_and_extract",
        "repository_ctx.download_and_extract",
        "repository_ctx.download_and_extract(url, output='', sha256='', **kwargs)",
    ),
    (
        "extract",
        "repository_ctx.extract",
        "repository_ctx.extract(archive, output='', stripPrefix='')",
    ),
    (
        "file",
        "repository_ctx.file",
        "repository_ctx.file(path, content='', executable=True, **kwargs)",
    ),
    (
        "read",
        "repository_ctx.read",
        "repository_ctx.read(path) -> string",
    ),
    (
        "symlink",
    ...
    (
        ...
        "repository_ctx.template(path, label, substitutions={}, executable=True)",
    ),
    (
        "which",
        "repository_ctx.which",
        "repository_ctx.which(program) -> path",
    ),
    (
        "workspace_root",
        "repository_ctx.workspace_root",
        "repository_ctx.workspace_root -> path",
    ),
    (
        "name",
        "repository_ctx.name",
        "repository_ctx.name -> string",
    ),
    (
        "attr",
        "repository_ctx.attr",
        "repository_ctx.attr -> struct",
    ),
    ("os", "repository_ctx.os", "repository_ctx.os -> struct"),
    (
        "environ",
        "repository_ctx.environ",
        "repository_ctx.environ -> dict[string, string]",
    ),
    (
        "getenv",
        "repository_ctx.getenv",
        "repository_ctx.getenv(name, default=None) -> string",
    ),
    (
        "delete",
    ...
    (
        "flag",
        "repository_ctx.flag",
        "repository_ctx.flag(name) -> string",
    ),
];
...
const PROVIDER_MEMBERS: &[(&str, &str, &str)] = &[
    ...
    (
        ...
        "cc_info.compilation_context -> CcCompilationContext",
    ),
    (
        "linking_context",
        "cc_info.linking_context",
        "cc_info.linking_context -> CcLinkingContext",
    ),
    // PyInfo (python rules)
    (
        "transitive_sources",
        "py_info.transitive_sources",
        "py_info.transitive_sources -> depset[File]",
    ),
    (
        "uses_shared_libraries",
        "py_info.uses_shared_libraries",
        "py_info.uses_shared_libraries -> bool",
    ),
    (
        "imports",
        "py_info.imports",
        "py_info.imports -> depset[string]",
    ),
    (
        "has_py2_only_sources",
        "py_info.has_py2_only_sources",
        "py_info.has_py2_only_sources -> bool",
    ),
    (
        "has_py3_only_sources",
        "py_info.has_py3_only_sources",
        "py_info.has_py3_only_sources -> bool",
    ),
    // DefaultInfo
    (
        "files",
        "default_info.files",
        "default_info.files -> depset[File]",
    ),
    (
        "files_to_run",
        "default_info.files_to_run",
        "default_info.files_to_run -> FilesToRun",
    ),
    (
        "data_runfiles",
        "default_info.data_runfiles",
        "default_info.data_runfiles -> runfiles",
    ),
    (
        "default_runfiles",
        "default_info.default_runfiles",
        "default_info.default_runfiles -> runfiles",
    ),
];
...
const RESULT_TYPE_MEMBERS: &[(&str, &str, &str)] = &[
    ...
    (
        ...
        "exec_result.return_code -> int",
    ),
    (
        "stdout",
        "exec_result.stdout",
        "exec_result.stdout -> string",
    ),
    (
        "stderr",
        "exec_result.stderr",
        "exec_result.stderr -> string",
    ),
];
...
const TEST_RESULT_MEMBERS: &[(&str, &str, &str)] = &[
    (
        ...
        "result.describe_failure() -> string",
    ),
...
const TARGET_MEMBERS: &[(&str, &str, &str)] = &[
    ...
    (
        ...
        "target.default_outputs -> list[File]",
    ),
    (
        "data_runfiles",
        "target.data_runfiles",
        "target.data_runfiles -> runfiles",
    ),
    (
        "default_runfiles",
        "target.default_runfiles",
        "target.default_runfiles -> runfiles",
    ),
    ...
    (
        "provider",
        "target.provider",
        "target.provider(provider_type) -> any",
    ),
    (
        "providers",
        "target.providers",
    ...
    (
        "files_to_run",
        "target.files_to_run",
        "target.files_to_run -> FilesToRun",
    ),
    (
        "output_groups",
        "target.output_groups",
        "target.output_groups -> struct",
    ),
    (
        "aspect_ids",
        "target.aspect_ids",
        "target.aspect_ids -> list[string]",
    ),
];
...
const RUNFILES_MEMBERS: &[(&str, &str, &str)] = &[
    (
        "merge",
        "runfiles.merge",
        "runfiles.merge(other) -> runfiles",
    ),
    (
        "merge_all",
        "runfiles.merge_all",
        "runfiles.merge_all(others) -> runfiles",
    ),
    ("files", "runfiles.files", "runfiles.files -> depset[File]"),
    (
        "symlinks",
        "runfiles.symlinks",
        "runfiles.symlinks -> depset",
    ),
    (
        "root_symlinks",
        "runfiles.root_symlinks",
        "runfiles.root_symlinks -> depset",
    ),
    (
        "empty_filenames",
        "runfiles.empty_filenames",
        "runfiles.empty_filenames -> depset[string]",
    ),
];
...
const ARGS_MEMBERS: &[(&str, &str, &str)] = &[
    (
        "add",
        "args.add",
        "args.add(arg_name_or_value, value=None, ...) -> Args",
    ),
    (
        "add_all",
        "args.add_all",
        "args.add_all(arg_name_or_values, values=None, ...) -> Args",
    ),
    (
        "add_joined",
        "args.add_joined",
        "args.add_joined(arg_name_or_values, values=None, join_with, ...) -> Args",
    ),
    (
        "set_param_file_format",
        "args.set_param_file_format",
...
const ATTR_FACTORIES: &[(&str, &str, &str)] = &[
    ...
    (
        "label_list",
        "attr.label_list",
        "attr.label_list(...) -> attr",
    ),
    (
        "label_keyed_string_dict",
        "attr.label_keyed_string_dict",
        "attr.label_keyed_string_dict(...) -> attr",
    ),
    ("string", "attr.string", "attr.string(...) -> attr"),
    (
        "string_list",
        "attr.string_list",
        "attr.string_list(...) -> attr",
    ),
    (
        "string_dict",
        "attr.string_dict",
        "attr.string_dict(...) -> attr",
    ),
    (
        "string_list_dict",
        "attr.string_list_dict",
        "attr.string_list_dict(...) -> attr",
    ),
...
const CONFIG_FACTORIES: &[(&str, &str, &str)] = &[
    ...
    (
        "exec",
        "config.exec",
        "config.exec(exec_group=None) -> config",
    ),
    (
        "string",
        "config.string",
        "config.string(name, default) -> config",
    ),
    (
        "bool",
        "config.bool",
        "config.bool(name, default) -> config",
    ),
...
const ENV_MEMBERS: &[(&str, &str, &str)] = &[
    ...
    (
        "assert_equals",
        "env.assert_equals",
        "env.assert_equals(expected, actual)",
    ),
    ("fail", "env.fail", "env.fail(msg)"),
    ("ctx", "env.ctx", "env.ctx -> ctx"),
    (
        "analysistest_target",
        "env.analysistest_target",
        "env.analysistest_target -> Target",
    ),
];
...
const ENV_EXPECT_MEMBERS: &[(&str, &str, &str)] = &[
    (
        "that_str",
        "env_expect.that_str",
        "env_expect.that_str(value) -> env_str_subject",
    ),
    (
        "that_int",
        "env_expect.that_int",
        "env_expect.that_int(value) -> env_int_subject",
    ),
    (
        "that_bool",
        "env_expect.that_bool",
        "env_expect.that_bool(value) -> env_bool_subject",
    ),
    (
        "that_collection",
        "env_expect.that_collection",
        "env_expect.that_collection(value) -> env_collection_subject",
    ),
    (
        "that_file",
        "env_expect.that_file",
        "env_expect.that_file(value) -> env_file_subject",
    ),
    (
        "that_target",
        "env_expect.that_target",
        "env_expect.that_target(value) -> env_target_subject",
    ),
    (
        "that_depset_of_files",
        "env_expect.that_depset_of_files",
        "env_expect.that_depset_of_files(value) -> env_depset_subject",
    ),
];
...
const ENV_EXPECT_FLAT_ALIASES: &[(&str, &str, &str)] = &[
    (
        "that_str",
        "env.expect.that_str",
        "env.expect.that_str(value) -> env_str_subject",
    ),
    (
        "that_int",
        "env.expect.that_int",
        "env.expect.that_int(value) -> env_int_subject",
    ),
    (
        "that_bool",
        "env.expect.that_bool",
        "env.expect.that_bool(value) -> env_bool_subject",
    ),
    (
        "that_collection",
        "env.expect.that_collection",
        "env.expect.that_collection(value) -> env_collection_subject",
    ),
    (
        "that_file",
        "env.expect.that_file",
        "env.expect.that_file(value) -> env_file_subject",
    ),
    (
        "that_target",
        "env.expect.that_target",
        "env.expect.that_target(value) -> env_target_subject",
    ),
    (
        "that_depset_of_files",
        "env.expect.that_depset_of_files",
        "env.expect.that_depset_of_files(value) -> env_depset_subject",
    ),
];
...
#[cfg(test)]
#[path = "bazel_central_registry_tests.rs"]
mod tests;
