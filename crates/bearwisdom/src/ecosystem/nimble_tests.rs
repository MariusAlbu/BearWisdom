...
"#,
    ...
    let deps = parse_nimble_requires(&tmp);
    assert!(
        deps.contains(&"nimcrypto".to_string()),
        "nimcrypto missing: {deps:?}"
    );
    assert!(
        deps.contains(&"bearssl".to_string()),
        "bearssl missing: {deps:?}"
    );
    assert!(
        deps.contains(&"chronicles".to_string()),
        "chronicles missing: {deps:?}"
    );
    assert!(
        deps.contains(&"chronos".to_string()),
        "chronos missing: {deps:?}"
    );
    assert!(deps.contains(&"stew".to_string()), "stew missing: {deps:?}");
    assert!(
        deps.contains(&"results".to_string()),
        "results missing: {deps:?}"
    );
    assert!(
        deps.contains(&"serialization".to_string()),
        "serialization missing: {deps:?}"
    );
    assert!(
        !deps.contains(&"nim".to_string()),
        "nim should be excluded: {deps:?}"
    );
    let _ = std::fs::remove_dir_all(&tmp);
}
...
fn nim_module_to_path_tail_converts() {
    assert_eq!(
        nim_module_to_path_tail("strutils"),
        Some("strutils.nim".to_string())
    );
    assert_eq!(
        nim_module_to_path_tail("std/strutils"),
        Some("std/strutils.nim".to_string())
    );
}
