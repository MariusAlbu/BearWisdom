...
pub(crate) fn build_php_symbol_index(dep_roots: &[ExternalDepRoot]) -> SymbolLocationIndex {
    ...
    let per_file: Vec<Vec<(String, Option<String>, String, PathBuf)>> = work
        .par_iter()
        .map(|(module, wf)| {
            let Ok(src) = std::fs::read_to_string(&wf.absolute_path) else {
                return Vec::new();
            };
            let (names, ns) = scan_php_header(&src);
            names
                .into_iter()
                .map(|name| (module.clone(), ns.clone(), name, wf.absolute_path.clone()))
                .collect()
        })
        .collect();
    let mut index = SymbolLocationIndex::new();
    for batch in per_file {
        for (module, ns, name, file) in batch {
            // Offers key twice: by composer package name (`phpunit/phpunit`)
            // and by the file's declared namespace (`PHPUnit\Framework`) —
            // the namespace is the module evidence `use` statements put on
            // refs, so a namespace-tagged ask lands on the offer directly.
            if let Some(ns) = &ns {
                index.insert(ns.clone(), name.clone(), file.clone());
            }
...
/// class / interface / trait / enum / function / const names plus the file's
/// declared namespace. Nested class methods are left to the regular extractor
/// when the file is eventually pulled into the index.
fn scan_php_header(source: &str) -> (Vec<String>, Option<String>) {
    ...
    walk_php_decls(&root, bytes, &mut out, &mut ns, 0);
    (out, ns)
}
    ...
    for child in node.children(&mut cursor) {
        match child.kind() {
            ...
            "namespace_definition" => {
                // First declared namespace wins; both the semicolon and the
                // braced form carry it in the `name` field.
                if ns.is_none() {
                    if let Some(n) = child
                        .child_by_field_name("name")
                        .or_else(|| find_first_name_child(&child, "namespace_name"))
                    {
                ...
                walk_php_decls(&child, bytes, out, ns, depth + 1);
            }
            "namespace_use_declaration" | "program" | "compound_statement" => {
