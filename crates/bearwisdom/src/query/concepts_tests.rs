...
fn discover_concepts_finds_namespace_prefixes() {
    ...
    assert!(
        discovered.contains(&"Microsoft.eShop".to_string()),
        "Should discover Microsoft.eShop"
    );
    assert!(
        discovered.contains(&"Top.Level".to_string()),
        "Should discover Top.Level"
    );
}
...
fn discover_concepts_is_idempotent() {
    ...
    assert_eq!(
        second.len(),
        0,
        "Second discovery should not create duplicates"
    );
}
...
fn discover_concepts_ignores_short_names() {
    ...
    assert!(
        discovered.is_empty(),
        "Two-segment names should not create concepts"
    );
}
