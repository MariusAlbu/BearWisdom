...
fn setup_graph(db: &Database) -> (i64, i64, i64, i64) {
    ...
    let b: i64 = conn
        ...
        .unwrap();
    let c: i64 = conn
        .query_row("SELECT id FROM symbols WHERE name='C'", [], |r| r.get(0))
        .unwrap();
    let d: i64 = conn
        .query_row("SELECT id FROM symbols WHERE name='D'", [], |r| r.get(0))
        .unwrap();
...
fn blast_radius_direct_callers_depth_1() {
    ...
    let result = blast_radius(&db, "A", 1, 500)
        .unwrap()
...
fn blast_radius_depth_2_includes_transitive() {
    ...
    let result = blast_radius(&db, "A", 2, 500)
        .unwrap()
        .expect("symbol not found");
...
fn blast_radius_full_chain() {
    ...
    let result = blast_radius(&db, "A", 10, 500)
        .unwrap()
        .expect("symbol not found");
    let names: Vec<&str> = result.affected.iter().map(|a| a.name.as_str()).collect();
...
fn blast_radius_symbol_with_no_callers() {
    ...
    let result = blast_radius(&db, "Lonely", 5, 500)
        .unwrap()
