...
use super::hierarchy::{
    files_breadcrumbs, symbols_breadcrumbs, HierarchyEdge, HierarchyNode, HierarchyResult,
    OptionalExt,
};
...
pub(super) fn files_level(
    ...
    let mut stmt = conn
        .prepare(&node_sql)
        .context("Failed to prepare files node query")?;

    let map_row =
        |row: &rusqlite::Row<'_>| -> rusqlite::Result<(i64, String, String, Option<i64>, u32)> {
            Ok((
                row.get(0)?,
                row.get(1)?,
                row.get(2)?,
    ...
    let mut estmt = conn
        .prepare_cached(edge_sql)
        .context("Failed to prepare files edge query")?;

    let emap_row =
        |row: &rusqlite::Row<'_>| -> rusqlite::Result<(String, String, String, u32, f64)> {
            Ok((
                row.get(0)?,
                row.get(1)?,
                row.get(2)?,
    ...
    for row in edge_rows {
        let (src_path, tgt_path, _kind, cnt, conf) =
            row.context("Failed to read files edge row")?;
    ...
    let edges: Vec<HierarchyEdge> = agg_edges
        .into_iter()
        .map(
            |((src_path, tgt_path), (weight, confidence))| HierarchyEdge {
...
pub(super) fn symbols_level(
    ...
    let mut stmt = conn
        .prepare(&node_sql)
        .context("Failed to prepare symbols node query")?;
    ...
    conn.execute_batch("CREATE TEMP TABLE IF NOT EXISTS _hier_syms (id INTEGER PRIMARY KEY)")
...

    {
        ...
        drop(ins);
        tx.commit()
            .context("Failed to commit hierarchy temp inserts")?;
    }
