        ...
        match child.kind() {
            ...
            "function_declaration" => {
                ...
                if let Some(sym_idx) = idx {
                    ...
                    extract_typed_params_as_symbols(&child, src, scope_tree, symbols, refs, Some(sym_idx));
                    if let Some(body) = child.child_by_field_name("body") {
...
fn extract_typed_params_as_symbols(
    func_node: &Node,
    src: &[u8],
    scope_tree: &ScopeTree,
    symbols: &mut Vec<ExtractedSymbol>,
    ...
    for i in 0..params.child_count() {
        ...
        let has_modifier = (0..param.child_count()).any(|j| {
            param.child(j).map(|c| c.kind() == "accessibility_modifier" || c.kind() == "readonly").unwrap_or(false)
        });
        if has_modifier {
            continue;
        }
