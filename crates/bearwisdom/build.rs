...
//
// Output layout:
//   src/languages/query_builtins/mod.rs     neutral generated data registry
//   src/languages/query_builtins/<lang>.rs  per-language BUILTINS + LOCALS_SCM
//
...
#[path = "build/extractor_digest.rs"]
mod extractor_digest;
#[path = "build/scm_builtins.rs"]
mod scm_builtins;

fn main() {
    let manifest = std::env::var("CARGO_MANIFEST_DIR").unwrap();
    let per_lang_dir = PathBuf::from(&manifest).join("src/languages/query_builtins");
    let dispatcher_path = per_lang_dir.join("mod.rs");
                ...
                for crate_entry in crate_dirs.flatten() {
                    ...
                    if highlights.exists() {
                        if let Ok(content) = fs::read_to_string(&highlights) {
                            ...
                            scm_builtins::extract(&content, names);
                        }
                    }
    ...
    extractor_digest::emit(Path::new(&manifest));
}
...
fn write_lang_file(
    ...
    out.push_str("pub(crate) const BUILTINS: &[&str] = &[\n");
    ...
    out.push_str("pub(crate) const LOCALS_SCM: Option<&str> = ");
...
/// Write generated, neutral data declarations. The language registry routes
/// these records only through the plugin that owns the requested grammar ID.
fn write_dispatcher(
    path: &Path,
    all_langs: &BTreeSet<String>,
    _builtins: &BTreeMap<String, Vec<String>>,
    _locals: &BTreeMap<String, String>,
) {
    let mut out = String::new();
    out.push_str(
        "// AUTO-GENERATED by build.rs — do not edit manually.\n\
         // Per-language query data. Language ownership is enforced by LanguagePlugin.\n\n",
    );
    ...
    out.push_str(
        "\n#[derive(Clone, Copy)]\n\
         ...
         \x20   pub(crate) builtins: &'static [&'static str],\n\
         \x20   pub(crate) locals_scm: Option<&'static str>,\n\
         }\n\n\
         impl Data {\n\
         \x20   pub(crate) const EMPTY: Self = Self { builtins: &[], locals_scm: None };\n\
         }\n\n\
         const ALL: &[(&str, Data)] = &[\n",
    );
    for lang in all_langs {
        let module = mod_ident(lang);
        out.push_str(&format!(
            "    (\"{lang}\", Data {{ builtins: {module}::BUILTINS, locals_scm: {module}::LOCALS_SCM }}),\n"
        ));
    }
    out.push_str(
        "];\n\n\
         pub(crate) fn for_language(language: &str) -> Data {\n\
         \x20   ALL.iter()\n\
         \x20       .find_map(|(id, data)| (*id == language).then_some(*data))\n\
         \x20       .unwrap_or(Data::EMPTY)\n\
         }\n",
    );
