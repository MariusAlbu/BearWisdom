...
- ✅ Ecosystem migration: all 18 source-based package ecosystems are demand-driven (go_mod, npm, pypi, maven, cargo, hex, spm, pub, rubygems, cran, composer, cabal, nimble, cpan, opam, luarocks, zig_pkg, plus godot_api as metadata-only). Only `clojure_core` (stdlib) and stdlib ecosystems (`go-stdlib`, `rust-stdlib`, etc.) remain on the legacy eager walk — explicit plan-scope deferral.
...
- ✅ `expand.rs` cleanup: the `resolve_symbol` fallback + its helpers (`rediscover_dep_roots_and_ecosystems`, `build_name_to_dep_idx`, `infer_dep_idx_for_fqn`, `file_matches_target`, `parse_dep_name_from_virtual_path`) are deleted; the module is ~200 lines instead of ~564, focused on the symbol-index-driven expansion path.
- ⏳ Connector flattening — infrastructure landed (`LanguagePlugin::extract_connection_points`, `connectors::from_plugins` bridge, `ConnectorRegistry::run_with_plugin_points`). 4 connectors migrated (graphql, proto, svelte, vue). ~46 remain — see `CONNECTOR_MIGRATION.md` for the per-connector checklist. This is genuinely multi-session work because each connector has its own detection logic + tests; source-scan-only ones follow a 4-step recipe; DB-lookup-dependent ones wait on a future `resolve_connection_points` post-parse fixup hook.
