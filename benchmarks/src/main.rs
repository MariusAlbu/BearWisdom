...
fn cmd_diagnose(project: &std::path::Path) -> Result<()> {
    ...
    // 1. Basic stats
    let (total_files, total_symbols, total_edges): (i64, i64, i64) = db.query_row(
        "SELECT
    ...
    let current_sampler_count: i64 = db.query_row(
        "SELECT COUNT(*) FROM symbols
    ...
    let fixed_sampler_count: i64 = db.query_row(
        "SELECT COUNT(*) FROM symbols
         WHERE kind IN ('interface', 'trait', 'class', 'struct', 'type_alias')
    ...
    let referenced_count: i64 = db.query_row(
    ...
    let hotspot_count: i64 = db.query_row(
        "SELECT COUNT(*) FROM (
