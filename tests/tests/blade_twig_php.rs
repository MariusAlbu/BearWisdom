...
"#,
    ...
    assert!(
        js_names.contains(&"onReady".to_string()),
        "expected onReady JS symbol, got {js_names:?}"
    );
    assert!(
        js_names.contains(&"onClick".to_string()),
...
"#,
    ...
    // Section symbols.
    let section_names: Vec<String> = db
        .prepare(
            "SELECT s.name FROM symbols s
         JOIN files f ON f.id = s.file_id
         WHERE f.path LIKE '%welcome.blade.php'
        ...
        .collect();
    assert!(
        section_names.contains(&"content".to_string()),
        "missing 'content' section, got {section_names:?}"
    );
    assert!(
        section_names.contains(&"title".to_string()),
    ...
    let imports_targets: Vec<String> = db
        .prepare(
        ...
        .collect();
    assert!(
        imports_targets.contains(&"layouts.app".to_string()),
        "missing @extends ref, got {imports_targets:?}"
    );
    assert!(
        imports_targets.contains(&"partials.footer".to_string()),
...
"#,
    ...
    let php_syms: Vec<String> = db
        ...
        .collect();
    assert!(
        php_syms.contains(&"helper".to_string()),
...
"#,
    ...
    let blocks: Vec<String> = db
        ...
        .collect();
    assert!(
        blocks.contains(&"base.content".to_string()),
        "missing base.content block, got {blocks:?}"
    );
    assert!(
        blocks.contains(&"base.footer".to_string()),
        "missing base.footer block, got {blocks:?}"
    );
    assert!(
        blocks.contains(&"page.content".to_string()),
    ...
    let refs: Vec<String> = db
        ...
        .collect();
    assert!(
        refs.contains(&"base".to_string())
            || refs.iter().any(|r| r.ends_with(".base") || r == "base"),
        "missing extends ref to base, got {refs:?}"
    );
    assert!(
        refs.iter()
            .any(|r| r == "partials.header" || r.ends_with(".partials.header")),
