...
use bearwisdom_profile::{
    detect_language, find_language, find_language_by_extension, scan, ScanOptions,
};
...
fn scan_csharp_project() {
    ...
    let langs: Vec<&str> = profile
        ...
        .collect();
    assert!(
        langs.contains(&"csharp"),
        "should detect C#, got: {langs:?}"
    );
}
...
fn scan_python_project() {
    ...
    assert!(
        ...
        "should detect Python, got: {langs:?}"
    );
}
...
fn scan_typescript_project() {
    ...
    assert!(
        ...
        "should detect TypeScript, got: {langs:?}"
    );
}
...
fn scan_empty_directory() {
    ...
    assert!(
        ...
        "empty dir should detect no languages"
    );
}
...
fn detect_language_known_extensions() {
    ...
    for (filename, expected_id) in cases {
        ...
        assert!(detected.is_some(), "should detect language for {filename}",);
        assert_eq!(
            detected.unwrap().id,
            expected_id,
