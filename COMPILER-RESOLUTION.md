...
- **2e** **Generic re-export following across languages** — ⚠️ (slice 1 ✅ `454e8b01`; slice 2 mapped).
  ...
  - **Slice 2 (the generic consumer) — mapped, not done.** Two pieces: (a) **Rust `pub use` tagging** — `walk_use_tree` (`rust_lang/calls_imports.rs`) hard-codes `is_reexport:false` at 6 emission sites and never inspects visibility; thread `detect_visibility` (`rust_lang/helpers.rs:52`) from the `use_declaration` node so `pub`/`pub(crate)`/`pub(super)` set true, private `use` stays false. (b) **the generic rung** — lift `follow_reexports` (`aliases.rs:293`) into a generic walk and add a `resolve_all` rung. Design notes from the understand pass: replace the TS-only `is_bare_specifier` skip (`aliases.rs:315`) with the generic `resolve_module_from`-returns-`None` check (external specs don't resolve to a project file); the **subtle part** is that the rung must resolve "which module each ref was imported from" to a file before walking its re-exports — TS does this in its own alias path, so the generic version needs that plumbing in `resolve_all`. A profile flag may be avoidable by self-gating the rung on `reexports_from(file)` being non-empty. **Soundness crux (the mandatory test): a private `use` must NOT resolve through a re-export hop** (Invariant #2). High leverage for Rust facade crates. **Python deferred** — re-export detection there is genuinely ambiguous without scanning `__all__` (treating every `__init__.py` relative import as a re-export over-follows).
