...
- **Tests**: 33 new `flow_cfg` tests (foundation Fact math, if/else, `&&`/`||`, loop back-edge, switch, def-kill, plus 12 per-language smoke tests — three narrowing assertions for Groovy/PHP/Ruby `instanceof`/`is_a?`, plus structural-only for Rust/Kotlin/Scala/C/Lua/R/Go/Java/C#/Python). **6020 bearwisdom lib tests, zero regressions**.
...
**Honest remaining (NOT done):**
- **Chain walker Union/Never dispatch.** `fact_string_at` projects `Union`/`Never` to `None`, so the consumer (chain walker via `local_type`) sees no narrowing for those cases. Genuine "use the union" would extend the consumer API (e.g., `local_type_fact(name) -> Option<NarrowedFact>`) and have the chain walker dispatch across union members (intersect resolutions, choose by discriminant, etc.). Real type-system work — hundreds of lines of `chain.rs` plus tests. Multi-session.
- **Rust if-let narrowing source** — investigated 2026-05-28 and reframed honestly: capturing the `if let` binding via tree-sitter is trivial, but producing a meaningful `narrowed_type` for the binding requires ADT/payload lookup (`Option<T>` → `T`, `MyEnum::Variant(P)` → `P`). Without type-system support a pure-syntactic recognizer could only write the constructor name into `narrowed_type`, which the consumer would then misuse as a real type. Properly slots in with the chain-walker Union dispatch — both touch the consumer surface.
- **Go `flow_config()` is `None` at runtime** (OOM workaround in `languages/go/mod.rs`) — `GO_CFG_KINDS` is wired and the type-switch path is structurally sound (proved with a synthesized-narrowing smoke test), but until the assignment_query OOM is fixed Go inherits no actual narrowings at indexing time.
- **Go `discriminant_guard_query`** — empty in v1 (Go's tree-sitter type_arguments node structure varies between grammar releases). When wired, expression-switch case discriminants will narrow the scrutinee per case.
- **Remaining ~10 languages without `CfgNodeKinds`** — Dart, Swift, the smaller-volume tree (Bash, Perl, Haskell, Elixir, Erlang, OCaml, Hare, …). Per-language tables, each ~30–60 lines + a smoke test. Lower yield since most have no `type_guard_query` either.
- **Retire the interval `Narrowing` vec** — contingent on the chain walker Union dispatch (so consumers can fully consume the CFG) and the corpus-driving languages having `CfgNodeKinds` (they all do now). Once chain-walker Union lands, drop `FlowMeta.narrowings` / `LocalTypeCache.narrowings` and have `lookup` query only the CFG.
...
**State (2026-05-28):** the entire #1 externals block is done — EXT-2 slice-1 ✅ (`1f03bec9`), EXT-1 ✅ (`29a055a4`), EXT-3 ✅ (`fa0a1e43`), EXT-4 ✅ (`aad62601`); #2's QUAL-1b H.1+H.2 ✅ (`bf46353b`/`c4252e8b`) and BIND-1 ✅. **QUAL-5 ✅ (`de0d9820`)** — three-state external model. **INFER-1 ⚠️ (substantially landed):** foundation + four narrowing cases (if/else, `&&`/`||`, loop back-edge, switch) + **fourteen languages CFG-native** (TS, JS, Java, Python, C#, Rust, Go, C, PHP, Lua, Groovy, Scala, Kotlin, Ruby, R) — commits `afedb777` · `d45e21dd` · `402756cc` · `16940b8b` · `e405c157` · `c78db287` · `6bda447b`, +33 flow_cfg tests (12 per-language smoke tests; three of them — Groovy/PHP/Ruby — assert end-to-end CFG-driven narrowing), **6020 lib tests green**. Genuine remaining work on INFER-1: chain walker `Union`/`Never` dispatch (deep type-system change), Go's runtime flow_config (OOM workaround pinning Go to structural-only), Rust `if let` (needs ADT/payload lookup, not just a query), Dart/Swift/long-tail CfgNodeKinds, retiring the interval `Narrowing` vec.

**Single best next move:** chain walker `Union`/`Never` dispatch. Currently `fact_string_at` projects `Fact::Union` and `Fact::Never` to `None`, so the consumer falls back to the declared type even when the CFG has computed a richer answer. Extending the consumer API (`local_type_fact(name) -> Option<NarrowedFact>`) and the chain walker (dispatch across union members for member lookup, treat `Never` as "unreachable use") is what makes the architect's "real union types at joins" decision observable end-to-end — and is also the unlock for Rust `if let` narrowing (same consumer-surface change handles the ADT-payload lookup). Larger blast radius than INFER-1's foundation (touches `chain.rs` / `loop_body.rs` on the hot path); honestly a multi-session piece of work, but it unblocks the corpus-visible gain from INFER-1's CFG. Alternative (smaller, mechanical): fix Go's `flow_config()` OOM in `languages/go/mod.rs` to turn the Go type-switch path on at runtime — the CFG side is already wired.
