...
### EXT-2 — Emit external return/field types (D4 / S5)  ·  [generic]+[profile]  ·  ⚠️ (slice 1 + module-tagged TypeRef filter ✅; signature-coverage at closeout)
...
**Remaining:** verify real external method symbols actually carry signatures across ecosystems (extractor coverage — measurable only at closeout recapture, not now).
...
- **1b** prefix → library binding — ⚠️ (`000be634`). The extractor now keeps the import URI (`collect_dart_import_aliases`) and `bind_prefixed_refs` stamps `namespace_segments=[prefix]` + `module=Some(uri)` onto the name ref, so a prefixed external ref (`i0.Value` → drift/`dart:`) is **branded external** via `classify_external` instead of dropping the prefix and staying `unresolved_unknown` (the QUAL-5 three-state win). A prefixed ref that misses its module **declines** (never falls through to the same-file bare-name strategy — that would mis-bind a coincidental local symbol @1.0). **Remaining:** the project-local case (`i2.X` → `./x.dart`) is dead — no Dart `ModuleResolver` is registered, so `in_module_from` can't resolve relative Dart imports. Follow-on: add a Dart `ModuleResolver` + the resolve-side guard test.
