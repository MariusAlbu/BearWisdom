...
### INFER-8 — Argument-driven generic inference  ·  [generic]  ·  ✅ (terminal call)
...
**Scope boundary — terminal call only.** `call_args` lives on the terminal `ExtractedRef`, not per-`ChainSegment`, so the single-expression mid-chain walk-past (`repo.find(user).name` in one expression) and bare generic calls (`const x = genericFunc(user)`, which route to the bare-name resolver, not the chain walker) are **not** covered. Both are separate follow-ons: per-segment call-arg extraction (a `ChainSegment` field + per-language chain-builder change, the same shape as the INFER-4 prerequisite), and a bare-name-resolver INFER-8 site. The within-file `const x = recv.genericMethod(args); x.member` path **is** covered via the `loop_body.rs` forward-inference cache. 11 `unify_into` unit tests + 6 chain integration tests; 6,169 lib tests, 0 regressions.
