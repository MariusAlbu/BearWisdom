...
**The headline follow-on — type synthesized members so chains resolve** (cross-cutting, fixes getters AND the builder fluent chain). Today synthesized methods carry their return type in the signature only, and `parse_return_type_from_signature` (`chain_walker.rs:591`) reads the trailing-`):`/arrow forms, not Java's leading-return-type — and synthesized symbols emit no return-type `TypeRef` (the path real Java methods use, `build.rs:332`). So `user.getName().x()` and `User.builder().name(y).build()` resolve only the first hop. Fix: have `synthesize_symbols` also return the synthesized symbols' refs (return-type `TypeRef`s sourced at the new symbols), rebased on splice like `embedded_regions` — a seam-signature change. **Other follow-ons:** `@*Constructor`; C# source-gen; Kotlin `data class`; then OPTIONAL consolidation — migrate the inline Python `@dataclass` / Rust derive / C `#define` one-offs onto the seam.
