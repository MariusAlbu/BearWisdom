...
### EXT-2 — Emit external return/field types (D4 / S5)  ·  [generic]+[profile]  ·  ⚠️ (slices 1+2+3 ✅ — colon/arrow + module-tagged TypeRef + JVM-descriptor decode `5b9e0ab4`; non-JVM cross-ecosystem signature coverage verify-at-closeout)
...
### CODEGEN-1 — Generic post-extract symbol synthesis  ·  [hook]  ·  ⚠️ (seam + Java Lombok + Kotlin data class done; generic method-return element typing done across colon/arrow `3a8491df`, leading-form Java/C# `a18c704d`, Go `[]` + C# head + externals `0481fec0`)
...
### LANG-PY-1 — MRO  ·  [profile]+[generic]  ·  ❌ (decision-ready proposal)
Method resolution order for multiple inheritance. (Dynamic `__getattr__`/duck typing out of scope.)
**Re-tagged [hook]→[profile]+[generic]** after scoping: Python is `engine_primary`, so member lookup runs through the shared `find_on_chain` → `walk_up_with_args` (a plain BFS over `SupertypeGraph`), NOT any `languages/python` code. The fix is one `LanguageProfile` field `ancestor_order: { Bfs(default), C3 }` + one generic `SupertypeGraph::linearize_with_args` (C3 merge; Bfs arm delegates unchanged), with `find_on_chain` consulting `profile.ancestor_order`. Python sets `C3`; the other 4 engine-primary languages keep Bfs (byte-identical). Pure data + one generic algorithm, no hook. **Open decision:** a non-defaulted profile field ripples to ~94 profile literals — defaulted field vs. mechanical sweep. Effort M. Failing test: asymmetric diamond where C3 and BFS pick different overrides.

### LANG-SWIFT-1 — Protocol extensions  ·  [hook]  ·  ❌
Protocol extensions, retroactive conformance, `some`/`any`. Swift also lacks a real `swift_import` strategy (Appendix structural-surprise) — the correct fix is a scope-directed import strategy. An attempt that instead bolted `DefaultResolver::resolve_all()` onto the hook tail (`69d35851`) was **reverted** (`17afa91d`): it drags the generic whole-program fallbacks into Swift wholesale, unaudited — a broader behavior change than the scoped strategy.
...
Svelte (LANG-SVELTE-1 + BIND-2a), **Vue** (incl. vuetify `VCol`/`VBtn` via `vue/global_registry.rs:111`), **Astro** (`astro/extract.rs:116`), **Angular** (`angular/extract.rs:120`), **MDX** (`mdx/extract.rs:5`) — all ✅ (were mis-listed as pending). **Remaining: plain HTML custom-element tags** — `html/extract.rs:18` resolves script-block calls only, no PascalCase / custom-element component binding. An attempt that emitted `Calls` refs for every PascalCase/kebab tag (`b203ba1c`) was **reverted** (`b9f76bc9`): those refs resolved through the generic by-name path, manufacturing coincidental binds (Invariant #2 violation — e.g. library custom elements binding to unrelated same-named symbols). The right fix must bind tags through a scope-directed component registry, not by-name.

### CORPUS-2 — DSL / markup resolvers below the 99% gate  ·  [profile]+[hook]  ·  ❌
Languages whose resolvers are weak or absent and so fail the per-language 99% bar: **gsp** — ⚠️ now ON the engine (`8cf6c168`): `GspHooks::build_file_context` was missing so the engine never ran on gsp; added it + `resolve_ref` binding `<g:render template>` partials to their `.gsp` Class by path (strategy `gsp_template`, mirrors handlebars/markdown). **bicep** (44%), **vbnet** (46%), **matlab** (62%), **html** (66% — CORPUS-1 plain custom-element tags pending), **prolog** (74%), **r** (76%). (`astro` dropped — its 0% was stale.) Each must reach 99% through the shared engine + its profile data, not a bespoke walker. Numbers are pre-recapture (DOC-4) and indicative.
