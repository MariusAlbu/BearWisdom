...
| QUAL-2b (py/ruby/php/c) | Retire the 4 remaining walkers; PHP `root_type_access` bool; C typedefs via generic `expand_aliases` (no fn-pointer); `expand_current_type` rewrites without an env for non-generic aliases. −546 | `73a578b3` |
...
**This task decided whether BearWisdom is "one engine + data" or "94 resolvers."** Done: all 8 bespoke `MemberChain` walkers are retired onto `resolve_via_chain(&<LANG>_CHAIN_CONFIG)`.
- **2a** reroute apparatus deleted — ✅ (`a3815b4e`).
- **2b — retire the bespoke chain walkers — ✅.** The 8 highest-volume forks (`walk_typescript_chain` 480L, `walk_csharp_chain`, `walk_go_chain`, `walk_java_chain`, `walk_python_chain`, `walk_ruby_chain`, `walk_php_chain`, `walk_c_lang_chain`) are deleted; each language is now `<LANG>_CHAIN_CONFIG` **data** on the shared walker. The per-language deltas reduced to `ChainExtensions` bools (`expand_aliases` / `walk_inheritance` / `promote_external_qname` / `root_construction` / `extension_method_fallback` / `root_type_access`) — **one fn-pointer total** (`root_fallback`, the npm-globals ambient probe). Net **−1,163 production LOC**; 6,074 lib tests, zero regressions across all 8 slices. Commits `58ad2b24` · `329c5a51` · `c406434e` · `22c4adc7` · `73a578b3`. The 5 languages already on the generic walker (Dart/Kotlin/Scala/Swift/Starlark) stay on `ChainExtensions::NONE`. **13 languages now share one chain algorithm.** Migrating onto the shared ladder is widening-only for the weaker forks (deeper inheritance climb, `by_name`-prefix-unique 1.0 hop, `members_of`-final) — fires after base lookups miss, kind-gated; corpus-delta deferred to the DOC-4 recapture.
- **Ada is NOT a chain-walker fork (roadmap miscount corrected).** The earlier "9 walkers" figure wrongly counted Ada's `walk_field_chain`. Verified via `bw_grep`: Ada's extractor emits **no `MemberChain`s** (`chain: None` on every ref; no `ada/` file appears among `chain: Some(...)` emitters), so `resolve_via_chain` (requires ≥2 segments) would resolve nothing for Ada. `walk_field_chain` is a field-resolution helper *inside* Ada's flat-dotted-name resolver (`AdaResolver::resolve`) — a different concern. Forcing it onto the chain walker resolves nothing AND would need ~7 Ada-only flags (a gate violation). **Out of QUAL-2b scope.** Whether Ada's name resolver (and other languages' name-resolution tails) consolidate onto a shared NAME-resolution path is a separate future question, not the chain-walker work.
...
1. **QUAL-2b — consolidate the chain walkers — ✅ DONE.** All 8 `MemberChain` walkers retired onto `resolve_via_chain` (Ada is a name resolver, not a chain fork — out of scope). 13 languages on one algorithm, −1,163 production LOC, 1 fn-pointer, 0 regressions. The gate is enforced; everything below now lands as data/capability on the survivor.
...
**State (2026-06-02):** spine done — externals (EXT-1 / EXT-2 slice-1 / EXT-3 / EXT-4), three-state metric (QUAL-5), CFG narrowing with end-to-end `Union` dispatch across 14 languages (INFER-1), binder shadowing (BIND-1), visibility-blind over-resolve (BIND-3). **QUAL-2 ✅** — all 8 chain-walker forks retired onto `resolve_via_chain`: **13 languages on one algorithm, −1,163 production LOC, 1 fn-pointer, 6,074 lib tests green, 0 regressions** (commits `58ad2b24` · `329c5a51` · `c406434e` · `22c4adc7` · `73a578b3`). The "one engine" invariant is now **true** for chain resolution. Newly tracked after the 2026-06-02 architecture audit: **INFER-8/9/10, BIND-2e, BIND-4, CODEGEN-1**; re-scoped **INFER-7, LANG-CPP-1**; corrected **LANG-DART-1c ✅, CORPUS-1 (HTML only), CORPUS-2 (astro dropped)**. Remaining: the §B inference spine, CODEGEN-1, the generic binder items (BIND-2e/4), `LANG-*` as data, then the DOC-4 closeout recapture (the per-language 99% scoreboard).
