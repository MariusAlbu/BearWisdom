...
### FACADE-1 — `SymbolView`: one accessor over identity + types + containment  ·  [generic]  ·  ⚠️ (facade + first consumer ✅; migration backlog open)
**The smell.** Resolution code hand-correlates three projections of the same symbol table: `by_name` → `SymbolTypeMap::get(id)` (param/return types) → `ContainingScope` (ancestry). All three are built **together** up front in `build_with_context` — for project code there is no temporal split (lazy hydration is the *externals* story, not internal identity-vs-types). So the fragmentation is ergonomic debt, not a storage necessity. **Roslyn's `ISymbol` is a *facade* over separately-sourced data, not one fat record** — the faithful fix is a facade, NOT a physical merge (a merge fights cache-locality: the ranking inner loop wants dense `param_types`, not containment dragged through cache).
**Slice 1 done (`2ae689c4`).** `SymbolView<'a>` = `{ &SymbolInfo, &SymbolTypeMap, Option<&ContainingScope> }` with `param_types`/`return_type`/`declared_type`/`generic_params` (honest tri-state — `None`=unhydrated) and `containing_type`/`containing_namespace`. First consumers: the extracted `arg_assignable_candidates` (chain dispatch reads param types through the facade) and BIND-4 (above).
**Consumer backlog — migrate the hand-correlation onto the facade, honest per class:**
- **Type-half (`symbol_types.get(id)`) — 7 sites, cheap/mechanical.** `chain.rs:571,873,941` · `members.rs:350` · `pattern.rs:163` · `subtype.rs:466` → `view.param_types()/return_type()/declared_type()`. (`dispatch.rs:106,194` already migrated.)
- **Containment-half, "symbol-in-hand splits its own qname" — 18 sites, cheap.** From the string-op audit: leaf cases read `sym.name` (already on `SymbolInfo` — no facade needed): `ada/hooks.rs:327,377,511,539` + `ada/chain.rs:95,154,209,246`. Parent/namespace cases read `view.containing_namespace()`: `go/{hooks.rs:301,309,predicates.rs:12}` · `rust_lang/predicates.rs:67,87` · `chain.rs:1113` · `augment.rs:102` (mirror `build.rs:151`'s structural `parent_index` path) · `scope_tree.rs:212`. (`build.rs:153` already correct — qname-split is its fallback arm.)
- **ScopeArena threading (enablement, gates the containment-half).** The facade's containment accessors need a `ContainingScope`; the `ScopeArena` + an `id→ScopeId` map must be threaded into resolution (`ChainWalker`/`Engine`) before `containing_namespace()` is usable at resolve time. BIND-4's scope filter uses the `scope_path` *field* as a stand-in pending this. **Do this before the containment-half migration.**
- **Bucket 2 — string→`TypeId` migration arc — ~128 sites, NOT a facade edit.** Bare `target_name`/`module_path`/FQN strings split because no structured type exists at that point. Dissolved upstream: intern types and carry structured refs (a `Ref` with separated `namespace_segments`+leaf, `TypeId`) so per-language hook entry points stop receiving/splitting flat strings. Large, gradual — its own tracked arc (continues the `symbol_types.rs` string-keyed→id-keyed direction). Do **not** present as cheap.
- **Bucket 3 — irreducible boundary parse — ~89 sites, low priority.** Extractors/walkers parsing a dotted token from source/jar/manifest. Only refinement: walk the AST's segment child-nodes once instead of `node_text`→re-split. Cannot reach zero.
**Deps:** — (slice 1 standalone). ScopeArena threading unblocks the containment-half; bucket 2 rides the broader `TypeId` migration.
