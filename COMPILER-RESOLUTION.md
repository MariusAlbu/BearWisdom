...
**Slice 1 done — the seam + first recognizer (`4f2dae31`).** The seam is `LanguagePlugin::synthesize_symbols(source, symbols, refs) -> Vec<ExtractedSymbol>` (no-op default, mirrors `embedded_regions`), invoked in `parse_file` after `filter_local_refs` and before the embedded splice — at EXTRACT time, so synthesized symbols get FK-safe ids (reconciling the roadmap's "LanguageEngineHooks" wording with `engine.rs:99-104`, which reserves the resolve-time hook for no-DB-id signals; the seam went on `LanguagePlugin`). Returned symbols carry `qualified_name` + `scope_path`; member parenting is rebuilt from `qualified_name` so `parent_index` stays `None`. First recognizer: **Java Lombok** — `@Data`/`@Value`/`@Getter`/`@Setter` on a class synthesize `getX`/`setX` (`isX` for a primitive boolean) per field, read purely from the extracted class/field symbols + annotation refs (no re-parse; the field type rides in its `signature`); a hand-written method of the same qname wins. Type-level only (field-level `@Getter` is ambiguous at the ref level — a field-level annotation and a field whose type is named `Value`/`Getter` are both a field-sourced `TypeRef`; the annotation-vs-type discriminator is that an annotation ref's `byte_offset` points at the `@`). 13 Java tests incl. an end-to-end `getName()` bind. **Remaining (follow-on slices):** `@Builder`'s nested builder class + `@*Constructor`; C# source-gen; Kotlin `data class`; then OPTIONAL consolidation — migrate the inline Python `@dataclass` / Rust derive / C `#define` one-offs onto the seam.
