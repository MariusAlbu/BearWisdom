...
### INFER-1 — Control-flow-graph-based narrowing  ·  [generic]  ·  ⚠️ (foundation + 4 cases + 6 languages ✅)
...
**Landed (commits `afedb777` · `d45e21dd` · `402756cc` · `16940b8b` · `e405c157`):**
...
- **Per-language `CfgNodeKinds` tables** — `TS_CFG_KINDS` · `JAVA_CFG_KINDS` · `PYTHON_CFG_KINDS` · `CSHARP_CFG_KINDS` · `GO_CFG_KINDS` · `RUST_CFG_KINDS` (six tables; structural CFG for all six). Narrowings inherit per-language via `guards_for_range(meta.narrowings, body_range)` — every language whose `type_guard_query` lands narrowings on guarded scopes gets CFG-native narrowing through the shared pipeline, no per-language condition parsing.
- **Tests**: 25 new `flow_cfg` tests (foundation Fact math, if/else, `&&`/`||`, loop back-edge, switch, def-kill, plus 4 per-language smoke tests). **6011 bearwisdom lib tests, zero regressions** — the hot-path rewire to `LocalTypeCache::lookup` is regression-free.
...
- **Remaining per-language `CfgNodeKinds`** — ~10 languages still on the interval path (Kotlin, Ruby, Scala, PHP, Lua, Dart, Swift, Groovy, R, C). Each is mechanical (table + smoke test) but adds up.
- **Rust if-let narrowing source** — Rust's `flow_config.type_guard_query` is currently empty (narrowings come from `flow_binding_decl_type` for `let x: T` annotations, not from `if let`). To make Rust CFG-native for `if let`, add a Rust `type_guard_query` recognizing `let_condition` patterns; the existing CFG pipeline then attaches the narrowing automatically.
- **Go `type_switch_statement`** — `CfgNodeKinds.switch_kind` is a single `&'static str`; Go's type-switch (the dominant narrowing source) would need it widened to a slice.
- **Retire the interval `Narrowing` vec** — contingent on the chain walker Union dispatch (so consumers can fully consume the CFG) and all corpus languages having `CfgNodeKinds`. Once those land, drop `FlowMeta.narrowings` / `LocalTypeCache.narrowings` and have `lookup` query only the CFG.
...
**State (2026-05-28):** the entire #1 externals block is done — EXT-2 slice-1 ✅ (`1f03bec9`), EXT-1 ✅ (`29a055a4`), EXT-3 ✅ (`fa0a1e43`), EXT-4 ✅ (`aad62601`); #2's QUAL-1b H.1+H.2 ✅ (`bf46353b`/`c4252e8b`) and BIND-1 ✅. **QUAL-5 ✅ (`de0d9820`)** — three-state external model. **INFER-1 ⚠️ (substantially landed):** foundation + four narrowing cases (if/else, `&&`/`||`, loop back-edge, switch) + six languages CFG-native (TS/JS, Java, Python, C#, Go, Rust structural) — commits `afedb777` · `d45e21dd` · `402756cc` · `16940b8b` · `e405c157`, +25 flow_cfg tests, 6011 lib tests green. Genuine remaining work on INFER-1: chain walker `Union`/`Never` dispatch (deep type-system change), the other ~10 corpus languages' `CfgNodeKinds` tables (mechanical), and retiring the interval `Narrowing` vec entirely (contingent on the first two).

**Single best next move:** chain walker `Union`/`Never` dispatch. Currently `fact_string_at` projects `Fact::Union` and `Fact::Never` to `None`, so the consumer falls back to the declared type even when the CFG has computed a richer answer. Extending the consumer API (`local_type_fact(name) -> Option<NarrowedFact>`) and the chain walker (dispatch across union members for member lookup, treat `Never` as "unreachable use") is what makes the architect's "real union types at joins" decision observable end-to-end. Larger blast radius than INFER-1's foundation (touches `chain.rs` / `loop_body.rs` on the hot path); honestly a multi-session piece of work, but it unblocks the corpus-visible gain from INFER-1's CFG. Alternative: grind out remaining per-language `CfgNodeKinds` tables and a Rust `type_guard_query` for `if let` (smaller, mechanical, expands coverage without changing the consumer model).
