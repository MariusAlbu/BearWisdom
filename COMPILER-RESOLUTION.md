...
**Fix (folds into QUAL-1b):** split the family — keep the **true-global** subset (Python `print`/`len`, Kotlin `listOf`/`apply`: resolvable in global scope, no import) as legitimate scope-directed binding; gate the **import-requiring** subset (Flutter/Android/JDK types) on an actual import binding (BIND-2/EXT-1). Under QUAL-5 the gated misses become `external_known_unhydrated` / `unresolved`, not `resolved`. Per-language engine % for Dart/Swift/Kotlin is inflated until this lands — but there is no fabrication to unwind, so it does **not** redirect the sequence.
