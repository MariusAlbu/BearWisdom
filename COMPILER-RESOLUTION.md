...
**Remaining:** the other 9 languages emit `CallArg::Other → Unknown` (no-op) until their `calls.rs` extraction is wired — legitimate per-language extraction work, not resolution drift.
...
**State (2026-06-02):** foundations done — externals (EXT-1 / EXT-2 slice-1 / EXT-3 / EXT-4), three-state metric (QUAL-5), CFG narrowing with end-to-end `Union` dispatch across 14 languages (INFER-1), binder shadowing (BIND-1), visibility-blind over-resolve (BIND-3). **QUAL-2 ✅** — all **9** chain-walker forks retired onto `resolve_via_chain`: **14 languages on one algorithm, −1,294 production LOC, 1 fn-pointer, 6,100 lib tests green, 0 regressions** (commits `58ad2b24` · `329c5a51` · `c406434e` · `22c4adc7` · `73a578b3` · `b2c20fa3`). The "one engine" invariant is now **true** for chain resolution. **INFER-5 ✅** (`d7626b76`) + **INFER-10 ✅** (`a948e965`, alias expansion for 6 languages). **Expression-typing track (2026-06-02):** the `CallArg`-recursive prerequisite landed (`32104a9d`/`8b5fd790`), so **INFER-4 ✅ for TS** (9 langs pending extraction) and **INFER-8 ✅** (arg-driven generics, terminal call) — `unify_into` + the `chain.rs` G2 branch, adversarially reviewed (one missed-inference gap found and fixed: owner params are bindable, `is_none`-guarded). Newly tracked after the 2026-06-02 audit: **INFER-9, BIND-2e, BIND-4, CODEGEN-1**; re-scoped **INFER-7, LANG-CPP-1**; corrected **LANG-DART-1c ✅, CORPUS-1 (HTML only), CORPUS-2 (astro dropped)**. Remaining: INFER-9, the `CallArg` breadth fan-out to 9 languages, the INFER-8 per-segment-args follow-on (bare-name site ✅ `6746b048`), CODEGEN-1, BIND-2e/4, `LANG-*` as data, DOC-4 closeout recapture.

**Single best next move:** the **`CallArg` extraction fan-out to the other 9 languages.** INFER-4 and INFER-8 are done as *algorithm*, but only TS *extracts* the recursive `CallArg` variants; the other 9 emit `CallArg::Other → Unknown`, so the whole arg-driven-inference spine (INFER-4 expression typing + INFER-8 terminal + bare-name binding) is dark for them. Wiring each language's `calls.rs` lights both up at once, per language — parallelizable, one slice per language, lower per-unit risk. Deferred: the INFER-8 per-segment `ChainSegment` call-args follow-on (single-expression mid-chain `repo.find(u).name`) — a ~30-site schema ripple for a form the multi-statement and bare-call paths already overlap; revisit only if DOC-4 shows it matters. (QUAL-2b, INFER-4/5/8/10, INFER-3/INFER-2, and the INFER-8 bare-name site — prior single-best-moves — are ✅ done.)
